...
ROWS = [
    ...
    dict(id='R-24', kind='FEATURE', by='operator', asOf='2026-09-08',
         text='The IRT export carries the SPY book’s top five like the SPX book’s — S2–S5 after the SPY King — at Skylit’s prices; every node line (G, S, the NQ G rows) carries its signed %King (`G3 -56%`, `S2 +55%`) and wears its polarity: yellow and its derivatives for +gamma, purple and its derivatives for −gamma (the SPY shades the palest). The Kings keep their bare labels.',
         changes='irtSpyTop (the SPY book’s next four by |v| after the King, from the same payload the SPY KING row reads, signed pct, no strength floor — NODES = 5 draws the top five whatever their size); S2..S5 after the SPY KING row, width 1, solid, es via skylitFutPx, held day-scoped under GS, IRT_LAST.sWhy; irtPctTag (the signed %); irtNodeCol + IRT_COLORS.gpos/gneg/spos/sneg (the polarity scheme, the book by shade); the G rows and the NQ G rows through the same two helpers; irtGHeld accepts S labels; the gear line names the S rows; the helpers typeof-guarded inside irtBuildCsv (degrade, do not depend)',
         evidence='his words 2026-09-08: “that spy node is important because it is a big node.. i currently have top 5 nodes for spx. Im thinking of having the top 5 for spy as well” · “i updated to 5, which shows me the top 5, so is that showing me the top 5 from both spx and spy” (it does — the feed returns nodes=5 per borrowed book) · “ok lets go with this” · “also add the % to each of the node lines except the king” · “add a + or - too so i know the polarity” · “redo the color schemes for the nodes, using yellow and its derivatives for positive nodes and purple and its derivatives for negative gamma nodes”; the 7690 he could not find on IRT was SPY 767, a SPY-book node the export never carried (R-13 was the SPX top-5)',
         status='implemented', version='15.83', why='his ask, four instructions in a row; shipped in v15.83'),
    dict(id='R-25', kind='DATA', by='operator', asOf='2026-09-08',
         text='THE TAP RECORD — the level study: one row per tap of an EXPORTED line (SPX KING / XG2–XG5, SPY KING / SG2–SG5 on ES; QQQ KING / G2–G5 on NQ) plus two controls (the rest of the 0DTE book ≥ 20%, and the midpoint between adjacent lines), stamped with book · rank · signed % · the conditions at the tap (tap number, FRESH / TESTED / DELIVERED / DECAYING, growth into the tap and day-over-day, King distance, gatekeepers, the pocket behind, the other book’s line within 5 pts, his key levels, approach speed and flush, regime, trinity, the clock, before / after the first extreme) and scored in trading terms (broke / mfe / mae / reach / R; DEFLECT · PIN · BREAK at 10 and 20 bars; was it the extreme; did the move resume) — nightly, on Analysis, by book × rank and by each condition with n, always beside the control.',
         changes='a recorder module (the tap detector + the outcome filler, pure, on fixture bars), an IndexedDB store `taps` mirrored into the day file, a nightly reader, one table on Analysis; the 110 registry studies whose corpus is “tap record” start reading from it; H11 (fresh + growing deflects more than tested or shedding, both more than the midpoint) registered the day the record starts; nothing on the face until the report’s mockup',
         evidence='his words 2026-09-08: “are you running a study on the gamma levels deflection. basically testing spx and spy and qqq top 5 gamma levels and their ability to deflect and if they do deflect, under what conditions and if they dont deflect, under what conditions” · “the purpose of the study is to identify what makes gamma levels work as deflectors of price for the purpose of trading reversals from levels” · “yes” to the design; the deflection ledger has no failures in it (no denominator), the node ledger is per bar on a mixed-window book (F-22); the first hand read of 2026-09-08 (34 taps: ranks 1–3 deflected 15 of 22, ranks 4–5 3 of 12 with all three breaks) is one day, not a verdict',
         status='proposed'),
    dict(id='R-26', kind='FEATURE', by='operator', asOf='2026-09-08',
         text='The IRT node lines are labelled by book: the SPX book’s XG2–XG5, the SPY book’s SG2–SG5 (the signed % follows as before: `XG3 -56%`, `SG2 +55%`); the Kings keep their names; the NQ symbol’s QQQ lines stay G2–G5 until he says otherwise.',
         changes='irtSpyTop labels SG2..SG5; the ES G rows XG2..XG5; irtGHeld accepts XG / SG (ES) and G (NQ), so a pre-v15.85 S2 hold is dropped and the next fresh book re-latches; the gear’s IRT line says “SG rows”; the QQQ King on NQ only (v15.80, R-18) pinned again on his word',
         evidence='his words 2026-09-08 (evening): “in the irt export, the spy gama level cand be SG2 SG3 etc., and the spx levels can be XG2 and XG3 etc.. make this change. also note the qqq king should only be on the nq not the es.”',
         status='implemented', version='15.85', why='an unambiguous instruction; shipped in v15.85 — the one open word is the NQ symbol’s QQQ labels (G or QG), asked'),
]
