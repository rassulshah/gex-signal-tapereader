...
ROWS = [
    ...
    dict(id='R-22', kind='DATA', by='review', asOf='2026-09-08',
         ...
         changes='repoExportDay / buildDayExport merge FEAT_ARCHIVE (IDB, every resolved record) with the localStorage queue for the date; recorderSave keeps snaps out of localStorage (the archive has them); the nightly and the review score the morning for the first time since 08-25',
         evidence='F-20, mechanism found live 2026-09-08: the localStorage budget (3600 KB) shed the queue’s first bar from 08:36 to 09:00 by 10:44 CT (shed 1, recorder 3542 KB); the exported snapshots come from IDB, which is why they looked intact; F-10c named the archive on 08-31'),
