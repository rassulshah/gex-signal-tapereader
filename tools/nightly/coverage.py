...
def corpus_counts(root=ROOT):
    ...
    # the tap record: not built (v15.94, after the RTH open took v15.93) — a file under data/taps/ will be counted here the day it exists
    ...
    put('tap', n_tap, len(taps) or None, (os.path.basename(taps[0])[:10] if taps else None), 'THE TAP RECORD — not recorded yet (v15.94)' if not taps else 'data/taps/<day>.json')
