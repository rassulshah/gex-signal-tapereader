...
def corpus_status(studies, counts, asof, answered=()):
    ...
    for x in _flat(studies):
        ...
        if not c or c in ('live', 'register', 'cut', '?'):
            continue
        if c == 'tap':
            out[x['id']] = dict(status='WAITING', line='WAITING · the tap record: 0 of %d taps — not recorded yet (v15.94) · nightly %s' % (need, asof), keepResult=True, n=0, asOf=asof, by='nightly')
            continue
...
def selftest():
    ...
    assert CS['K1.4']['status'] == 'WAITING' and CS['K1.4']['line'].startswith('WAITING · the tap record: 0 of 40 taps — not recorded yet (v15.94)') and CS['K1.4']['keepResult'] is True
