...
- [ ] **(v15.57) `node test_installer_manifest.js` green** — every file the panel fetches from GitHub is in the installer; the cloud cannot push, so a file not in the manifest does not exist.
