...
// ---- 4 · the Windows task: the scripts, line by line -----------------------------------------------------------------------
{
  ...
  ok(/cd \/d "%REPO%"\r\n(REM [^\r]*\r\n)?set PYTHONUTF8=1\r\nset PYTHONIOENCODING=utf-8\r\n!PY! tools\\nightly\\tick\.py >> "%LOGF%" 2>&1/.test(bat) && /if "!RC!"=="0" echo .* nightly ran/.test(bat) && /if "!RC!"=="1" echo .* NIGHTLY FAILED/.test(bat) && /REM RC 3 = nothing to do: silent/.test(bat),'4d it runs tick.py from the repo (UTF-8 out, v15.72b), logs a run and a failure, and writes NOTHING for a no-op (144 ticks a day)');
