...
// ---- 4 · THE RECORD ---------------------------------------------------------------------------------------------------
{
  ...
  { const r89=(P.roadmap||[]).find(r=>r.v==='15.89'); ok(r89 && /DATA TAB/.test(r89.title) && /shipped|next/.test(r89.status) && (P.roadmap||[]).find(r=>/^(15\.9[1-9]|16\.0\d)$/.test(r.v) && /TAP RECORD/.test(r.title)), '4c the plan: 15.89 the Data tab (shipped), the tap record after it (v15.99: King-chart fix took the 15.99 slot, tap record shifted to 16.00)', r89&&[r89.v, r89.status]); }
