...
// ---- 4 · the real shedders shed the right things ---------------------------------------------
{
  ...
  // ⚠ EXECUTED, NOT GREPPED. `/day\.snaps/` matched even with the snapshot branch mutated to
  // `if(false)` — the string was still in the source. That is failure pattern #8, and it would have
  // shipped the exact bug F-10 describes: shedding `feat` while 5,957 KB sat in `snaps`.
  {
    // brace-match the shedder out of recorderSave rather than guessing at a delimiter
    const st=rs.indexOf('function(d){');
    let dep=0, en=-1;
    for(let k=rs.indexOf('{',st);k<rs.length;k++){ if(rs[k]==='{')dep++; else if(rs[k]==='}'){dep--; if(dep===0){en=k;break;}} }
    const shedFn=eval('('+rs.slice(st,en+1)+')');
    const day={ snaps:{ SPY:new Array(200).fill({a:1}) }, feat:{ SPY:new Array(200).fill({b:2}) } };
    global.TODAY='2026-08-28';
    const d={ days:{} }; d.days[TODAY]=day;
    const before=day.snaps.SPY.length;
    shedFn(d);
    ok(day.snaps.SPY.length<before,
       's11 recorderSave actually SHEDS SNAPSHOTS — 5,957KB was mostly snaps, not feat',
       [before, day.snaps.SPY.length]);
  }
