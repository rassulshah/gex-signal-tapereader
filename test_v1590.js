...
// ---------- 3. the registry is the machine's output ----------
{
  ...
  ok(w.status==='WAITING' && /the tap record: 0 of 40 taps — not recorded yet \(v15\.9\d\)/.test(w.nightly) && r.status==='READY' && /kingroll: \d+ rows on hand since 2026-08-19 — no reader yet/.test(r.nightly) && b.status==='WAITING' && /book: \d+ of 30 sessions · ~\d+ sessions at 1\.0\/session/.test(b.nightly) && /^\d+ sessions$/.test(b.result), '3j WAITING with have / need / ETA, READY with "no reader yet" — the review\'s sentence kept beside them');
