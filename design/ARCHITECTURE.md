...
| **THE NIGHTLY — tools/nightly/run.py (+ tick.py, patterns.py, results.py, tape.py)** | his machine — the 'GEX nightly' Windows task (setup-gex-nightly.bat → tools/gex-nightly.bat, hidden, every 10 minutes; tick.py runs it only when the newest day file is newer than its log; Python 3 on the PATH) — and the cloud when I am in a session; no other model, no API | reads the register and every day file: verdicts per hypothesis read ONCE at minN, the pattern table (held rate by setup × book, and from v15.69 the objective outcomes turn / resume from the day's own bars), the tape coverage, the refreshed sweep tables, (v15.87 / v15.88) the futures corpus appended from EVERY day file (RTH → <day>.csv; from v15.90 the nights → <day>-night.csv) and ES / NQ BASERATES rebuilt, (v15.90) the sweep corpus refreshed from the vendor file + every CSV (SWEEPS.json appends like the base rates), (v15.89) the record counted — learning/coverage.json (every day file's signals, the corpora, and from v15.90 every corpus the registry names: have · per-session rate), new TRACK requests and items copied; (v15.87) tick.py also re-runs it when an installer has pasted older outputs over its log — by mtime, and by results.json's asOf against the log's day (2026-09-08: three installers after the 15:05 run put the 09-07 outputs back on GitHub); writes learning/log/<day>.json (ranOn: his machine / cloud) and then the study registry — every study whose number the log can answer gets it (results.py → learning/results.json, studies.json patched in place: result · status · by:'nightly' · asOf; a thin row keeps the review's sentence and shows the count so far; (v15.90) the scripted rows' sentences composed from SWEEPS.json and the day files' feature outcomes, and EVERY other row set WAITING — have / need / an ETA from coverage.json's counts — or READY — data on hand, no reader yet; H5 judged by its join; a withdrawn hypothesis said, never counted) and the Learn tab's rules (each rule's class, its numbers, the record's verdict — agrees / contradicts / thin) | within ~10 minutes of the day file (the task); the sync pushes it ≤ 2 minutes later; the panel re-fetches the registry and the log on its 10-minute check or a reload | his machine · cloud |
...
| **THE CORPORA — data/es-1min/ (the vendor's ES 284 sessions through 2026-08-21 and, from v15.88, its NQ 188 through 2026-08-27 — one contract each) + data/futures/<ES|NQ|GC|CL>/<day>.csv (the courier's Yahoo minutes, the continuous front month, from 2026-08-24) → BASERATES.json per market (ES 295 · NQ 195 on 2026-09-08)** | the repo, written by the nightly (v15.87) | the HOD/LOD tables: study-hodlod pools the vendor sessions and the Yahoo days with each session's source recorded (a day both hold comes from the vendor; the overlap is reported per market — ES none, NQ four days 08-24 → 08-27 read side by side: the clocks and extremes identical, the 08:27 open 0.5–7.5 pts apart); the sweep corpus (study-sweeps.py — ES 284 vendor sessions + the couriered sessions whose night the courier holds, 290 on 2026-09-09, one more a night since v15.90) cuts the night at 17:00 → the open (F-23), takes the prior day's levels only from the prior trading day (the four-day gap rule) and carries the Asia / London sessions and the weekly POC, on the tool grid (3-minute bars stamped by end, the open = the bar ending 08:30); ES 295 sessions and NQ 11 as of 2026-09-08, one more each session; the panel's boot literal is re-baked from the file at build time (tools/bake-hodlod.py) and the companion couriers the live file over it | nightly, after the day file (the newest day completes from the next day's courier window) | his machine (the nightly) · the cloud at build time (the bake) |
...
- **what:** the SPXW tape (strike, %King, the King's $K), the velocity table (every strike's size and d5/d15/d60/d1d for today's expiry), the Trinity panes (SPY · QQQ · SPXW · VIX: 100 strikes as %King + the King's $K), the headers (prices), the chart's 3-minute candles; (v15.81) the FUTURES books the page publishes — gex/levels?symbol=ES1 / NQ1, all derived: every SPY / SPXW / SPX (QQQ) strike already at the futures price by Skylit's live ratio, kept for the IRT export and self-fetched while it is on; (v15.88) the King is followed on the same tick — the v14.19 two-minute latch is off ("match skylit"); (v15.84) the panel's OWN SPY / QQQ book is one window and one breadth — the self-fetch pins exp_mode=current&exp_count=1&nodes=500 whatever gex/levels URL came last, and onFeed refuses a multi-expiration payload once a single-expiration book is held (F-22; FEED_REJECTS[sym].win counts them)
- **keys / functions:** LASTFEED · VEL · TAPE_CACHE · LADDER_CACHE · LASTFUTDER · FEED_REJECTS
...
- **how:** the companion only (the panel is @grant none and never touches Yahoo — test_futbars f30): Yahoo's v8 chart endpoint, <SYM>=F?interval=1m&range=5d — every 5 min in RTH, hourly off-hours; (v1.19) <SYM>=F?interval=5m&range=1mo for ES and NQ every 6 h (the weekly bars); ^VIX interval=1d&range=2y every 12 h
- **what:** ES and (v1.19) NQ the whole Globex day (ONH/ONL are the real overnight 17:00→08:29 CT; AHI/ALO 17:00→02:00, LHI/LLO 02:00→08:30), GC, CL RTH 1-minute bars; the prior ISO week's RTH 5-minute bars for PWH / PWL / WPOC; VIX daily closes
- **keys / functions:** gpts_futbars_v1 → futBarsLoad() (the day file's futBars) · gpts_futweek_v1 → futWeekLoad() → priorWeek() · gpts_vix_daily_v1
- **notes:** 1-minute history is ≤ 7 days at the source — a gap longer than that can never be recovered; the day file carries the bars so the corpus grows even when the courier missed a poll; 5-minute history reaches 60 days, which is why the weekly levels ride a second fetch
