...
| **THE CORPORA — data/es-1min/ (the vendor's ES 284 sessions, one contract, through 2026-08-21) + data/futures/<ES|NQ|GC|CL>/<day>.csv (the courier's Yahoo minutes, the continuous front month, from 2026-08-24) → BASERATES.json per market** | the repo, written by the nightly (v15.87) | the HOD/LOD tables: study-hodlod pools the vendor sessions and the Yahoo days with each session's source recorded (a day both hold comes from the vendor; the overlap is reported — none yet), on the tool grid (3-minute bars stamped by end, the open = the bar ending 08:30); ES 295 sessions and NQ 11 as of 2026-09-08, one more each session; the panel's boot literal is re-baked from the file at build time (tools/bake-hodlod.py) and the companion couriers the live file over it | nightly, after the day file (the newest day completes from the next day's courier window) | his machine (the nightly) · the cloud at build time (the bake) |
