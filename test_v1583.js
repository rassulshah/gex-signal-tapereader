...
// ---------- 0. the version ----------
ok(/@version\s+15\.83/.test(src) && /var GPTS_VERSION='15\.83';/.test(src), '0a v15.83 in both spots');
...
// ---------- 1. the helpers ----------
{
  ...
  ok(T.rows.length===4 && T.rows.map(r=>r.lbl+' '+r.k+' '+r.pct).join('|')==='S2 767 98|S3 766 80|S4 765 67|S5 769 40', '1f irtSpyTop: the next four by |v| after the King, S2..S5, signed pct', T.rows);
  ...
  ok(/\/\^\[GS\]\[2-5\]\$\/\.test/.test(ex('irtGHeld')), '1k the hold accepts S2..S5 (under its own key, GS)');
}
...
// ---------- 2. the export ----------
{
  ...
  // the S rows
  ok(lbl(e(/^S2/))==='S2 +98%' && px(e(/^S2/))===7690.0 && col(e(/^S2/))===IRT_COLORS.spos && wid(e(/^S2/))===1, '2a S2 = SPY 767 (+98%) at Skylit\'s 7689.97 → 7690.00, the palest yellow, width 1', [lbl(e(/^S2/)), px(e(/^S2/))]);
  ok(lbl(e(/^S3/))==='S3 +80%' && px(e(/^S3/))===7680.0 && lbl(e(/^S4/))==='S4 +67%' && px(e(/^S4/))===7670.0 && lbl(e(/^S5/))==='S5 +40%' && px(e(/^S5/))===7710.0, '2b S3 766 · S4 765 · S5 769 — the next four by size, at the derived rows\' prices', [lbl(e(/^S3/)),lbl(e(/^S4/)),lbl(e(/^S5/))]);
  ...
  ok(lbl(e(/^G2/))==='G2 +77%' && col(e(/^G2/))===IRT_COLORS.gpos && lbl(e(/^G3/))==='G3 -56%' && col(e(/^G3/))===IRT_COLORS.gneg, '2d G2 +77% yellow · G3 -56% purple — the sign is the polarity, the colour follows it', [lbl(e(/^G2/)), lbl(e(/^G3/))]);
  ok(lbl(e(/^G4/))==='G4 +31%' && lbl(e(/^G5/))==='G5 -27%' && col(e(/^G5/))===IRT_COLORS.gneg, '2e G4 +31% · G5 -27%');
  ...
  ok(/^live \(S2 767 98%, S3 766 80%, S4 765 67%, S5 769 40%\)$/.test(IRT_LAST.sWhy||''), '2j IRT_LAST.sWhy names the four', IRT_LAST.sWhy);
  ...
  ok(held.GS && held.GS.rows.length===4 && held.GS.rows[0].lbl==='S2' && held.GS.rows[0].pct===98 && held.GS.rows[0].k===767, '2l the S rows are latched under GS with their strike and signed pct', held.GS&&held.GS.rows[0]);
  ...
  ok(lbl(e2(/^S2/))==='S2 +98%' && px(e2(/^S2/))===7690.0 && /^held \d+m/.test(IRT_LAST.sWhy) && /^held/.test(IRT_LAST.spyWhy), '2m the SPY feed stale → the S rows (and the King) print from the hold at Skylit\'s prices, sWhy says held', [lbl(e2(/^S2/)), IRT_LAST.sWhy]);
  ...
  ok(!row(L3,'EPU26',/^S2/) && /no fresh SPY feed and nothing latched today/.test(IRT_LAST.sWhy), '2n nothing to hold and no feed → no S rows, no invention; sWhy says why', IRT_LAST.sWhy);
  global.LASTFEED={ SPY:{ ts:Date.now(), j:SPYJ }, QQQ:{ ts:Date.now(), j:{ levels:[{ s:722 }] } } };
  ...
  ok(px(row(L,'SPY',/^S2/))===767 && px(row(L,'SPY',/^S5/))===769, '2q the ETF symbol carries S2..S5 at the SPY strikes');
  // the gear line names the S rows
  ok(/\+\(IRT_LAST\.sWhy\?\(' \\u00b7 S rows '\+g3esc\(IRT_LAST\.sWhy\)\):''\)/.test(src), '2r the gear\'s IRT line says how the S rows were resolved');
}
...
// ---------- 3. the record ----------
{
  ...
  const rn=fs.readFileSync('session-state/latest-resume-note.md','utf8'); ok(/v15\.(8[3-9]|9\d)/.test(rn.slice(0,600)) && /S2/.test(rn), '3g the resume note is at v15.83 or later and names the S rows');
