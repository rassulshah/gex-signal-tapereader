...
(function(){
  // ⚠⚠ (v15.04) ONE TABLE, SO ONE ASSERTION. This used to split the section on the `g3dayg6` class
...
// ---- (v15.04) THE AGREED REMOVALS. Each was recorded as done and was still rendering. ---------
...
// (v15.04) the strip is superseded by ROW 3 — he asked for "a thrid row for the HL fields". Both
...
// ---- (v15.04) SLvl / TLvl: no IB, two on the face, the rest on the E row --------------------
(function(){
...
// ---- (v15.04) HIS SKETCH: narrow bar, annotations stacked ABOVE and BELOW, money in the body ---
...
(function(){
  ...
  // (v15.04) MUD sits on the side of the OPEN the session travelled — above on a red bar, below on
  ...
  // ---- (v15.04) the shape spine came BACK, and the reversal levels arrived --------------------
...
// ---- (v15.04) THE FILTER IS THE FEATURE -------------------------------------------------------
(function(){
  const RL=grab('revLevels').replace(/\/\/[^\n]*/g,'').replace(/\/\*[\s\S]*?\*\//g,'');
  ok(/atr\(sym\)\*rr/.test(RL), 'r1 the tolerance is ATR-scaled, not a fixed band');
  // ⚠⚠ (v15.04) ASYMMETRIC, like the deflection geometry. v14.99 used ONE symmetric tolerance,
  ...
  // (v15.04) the four levels that did not exist before
...
// ---- (v15.04) THE KING AT A MOMENT, NOT THE KING NOW ------------------------------------------
