...
ok(E5 && E5.date==='2026-09-08' && E5.legs.length===6 && E5.blind===null && /each should be counted as 1 deflection/.test(E5.his), '1b E005: six legs, taught (not scored), his words verbatim');
ok(E6 && E6.date==='2026-09-08' && E6.legs.length===4 && E6.blind===null && /NQ1/.test(E6.chart), '1c E006: four NQ legs, taught');
...
ok(E5.legs.filter(l=>l.kind==='LOD').length===1 && E5.legs.filter(l=>l.kind==='PULLBACK').length===5, '1e E005: five pullback turns and the LOD');
...
ok(E5.legs[5].factors.indexOf('new')>=0 && E5.legs[5].factors.indexOf('stack')>=0 && E5.legs[3].factors.indexOf('liq')>=0, '1h the LOD leg is new + stack; the flush leg carries liq');
ok(/NOT CIRCLED/.test(E5.open) && /09:03/.test(E5.open) && /day\.defl/.test(E5.open), '1i E005.open records what he did not circle and the panel ledger\'s miss');
...
ok(R('L5').n===10 && R('L5').agree===9 && R('L9').n===12 && R('L7').weak===13, '2f L5 / L9 / L7 counts moved with the ten circles', [R('L5').n, R('L9').n, R('L7').weak]);
