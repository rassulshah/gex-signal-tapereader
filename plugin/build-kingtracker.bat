@echo off
REM ===========================================================================
REM  build-kingtracker.bat  —  compile KingTracker.cpp -> lsKingTracker.dll (x64)  [KING TRACKER stepped lines]
REM
REM  EASIEST: double-click  compile-kingtracker.bat  (sets up the compiler for you).
...
where cl >nul 2>nul
if errorlevel 1 (
  echo [ERROR] cl.exe is not on PATH. Double-click compile-kingtracker.bat, or open an
  echo         "x64 Native Tools Command Prompt for VS" and run build-kingtracker.bat there.
  exit /b 1
)

echo Compiling KingTracker.cpp -^> %OUT% ...
cl /nologo /LD /EHsc /O2 /MD KingTracker.cpp /I "%SDK%\include" /Fe:%OUT% /link "%LIB143%"
if errorlevel 1 (
  echo.
...
del "%DLLDIR%\KingTracker.dll" 2>nul
