> ⚠⚠ **SNAPSHOT, SUPERSEDED 2026-08-31.** Kept because this note is the CASE STUDY for its own
> §0 warning. Its header was stamped **v15.09** — which is all `test_savedone` checks — while its
> body below still described **v14.80**. Nine versions of work (v14.81 → v15.09) never reached it,
> and every guard stayed green. **A version-keyed guard is satisfied by a stamp, not by content.**
> The live note is `session-state/latest-resume-note.md`.
