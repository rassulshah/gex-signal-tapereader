...
_written 2026-09-02, amended 2026-09-11 (v15.98, the King chart reads the census) · panel v15.98 · companion v1.19 · supersedes every earlier resume note (the v15.97 note is kept as 2026-09-11_resume-v15.97.md)_
...
# ⚠⚠ 2026-09-11 (latest) — v15.98: THE KING CHART READS THE ROLL CENSUS (krOf), AND IS HONEST WHEN EMPTY

**Panel is v15.98.** Three installs went into the King chart; v15.96 (view) and v15.97 (a second-of-day fix) both drew the
King lines FLAT. Root cause, found via his `__gptsDebug.kingTrack()` dump (every book `raw:[]`) + `__gptsDebug.ladders()`
(live 100-strike book): (1) I read `kingDay.moves` — a sparse "moved N× today" counter — instead of the King-rolls CENSUS
`krOf(book)` (`KRAW`; the frames in replay), the Atlas-comparable source the King lanes use; and (2) the census is gated OFF
unless the panel shows live truth in RTH (`recorderBlind()` — replay or a frozen last-session book), and today the panel was
PARKED until ~13:00, so no morning rolls were recorded.

**v15.98:** `kingSteps(book)` reads `krOf(book)`; the line is drawn ONLY from the first recorded King onward (the un-recorded
morning is BLANK, not a flat seed — that flat morning was the misleading part); an empty chart names the reason
(`recorderBlind()` → parked/replay → click LIVE); and a note says "King tracking began ~HH:MM" when it started mid-session.
His 13:27 screenshot confirmed it works once live (SPX K held 13:18/24/27, both lines stepping near 13:00).

**⚠ KEY TRUTH for the next context:** the King tracker records ONLY while the panel is LIVE in RTH. If the panel is parked
(recorderBlind), the King history for that period is LOST — not a chart bug. So the chart can only show rolls from when the
panel was live. The three king records: `kingDay.moves` (sparse counter — DON'T use), `KINGHIST.seq` (dense 3-min sample),
`krOf`/`KRAW` (the census — USE THIS, it's Atlas-comparable and covers SPXW).

**⚠ SCALE unchanged:** SPY King exact (× rr), SPX King approx (× dispScale, `~`). Tests: `test_v1596` 18.
**OPEN with him:** he asked to VERIFY the chart's kings against Atlas. The panel's kings ARE Skylit/Atlas's (that's the source),
so verifying = the chart line matches `krOf`/`kingTrack` matches what Atlas draws. I CANNOT browse Atlas from the cloud (his tab
isn't targetable; a second Atlas tab would double-record). Verify via his kingTrack dump vs Atlas, or after the close on a full
live day.
