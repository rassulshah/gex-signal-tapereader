...
// Parameter version 8 (v0.77, the regroup): the defaults ARE his settings (09-19 dialog); a v7 instance is reset to them once.
struct PIdx {
    ...
    int tnmode, tnstyle, tnwidth, ifstyle, magline;   // (v0.75) APPENDED: top nodes as bands / lines, their style + width, the IF level style, the Mag line
};
static PIdx PX;
...
struct Settings {
    ...
    int tnmode, tnstyle, tnwidth, ifstyle; bool magline;   // (v0.75)
...
public:
    ...
    bool scrambled(); void migrateScrambled(); void syncSettings();   // (v0.78)   // (v0.65) the repair-on-load (the version bump does not reset)
    ...
    void drawLevel(int lastBar, short lx, short rx, float price, COLOR col, const char* label,
                   bool extend, int style, int wpx, int lposEff, const Settings& S);   // (v0.75) style 0-2, 3 = label only
};
...
GammaProfile::GammaProfile() : cppExtension()
{
    ...
    cfg.book=4; cfg.width=90; cfg.side=0; cfg.detach=true; cfg.thick=2; cfg.round=true;   // (v0.77) his settings: Medium
    ...
    strncpy(cfg.kinglabel, "GPOC", sizeof(cfg.kinglabel)); cfg.kinglabel[sizeof(cfg.kinglabel)-1]=0;
    cfg.kline=true; cfg.cw=true; cfg.pw=true; cfg.flip=true; cfg.em=true;   // (v0.77) his EM H/L on
    cfg.lstyle=1; cfg.lpos=0; cfg.extk=false; cfg.topnodes=false; cfg.topstyle=1; cfg.spyking=false; cfg.rankmode=1; cfg.spywidth=120; lastOff=0.0f;
    ...
    cfg.bands=false; cfg.bandh=3; cfg.klinew=2;   // (v0.77) his: Node bands off
    cfg.tnmode=0; cfg.tnstyle=1; cfg.tnwidth=0; cfg.ifstyle=0; cfg.magline=false;   // (v0.75)
...
bool GammaProfile::scrambled()
{
    ...
    if (f < 1 || f > 200 || h < 0 || h > 100 || w < 20 || w > 1000 || t < 0 || t > 100) return true;   // (v0.77) w >= 20: a v7 instance puts its Side (0/1) in the SPX width slot
...
{
    // (v0.78) NO ONCE-GATE. IRT can call parmsLoad BEFORE a restored instance's saved values exist (the DS 0.16 lesson): the
    // check then sees the compiled defaults, passes, and the v7 values land afterwards unchecked — his 0.77 dialog on the
    // second open: SPX width 0 (= his old Side), SPY width 2 (= Medium), font 20 (= Threshold), Threshold 12860335 (= the
    // −Gamma colour), black swatches, tape off. The test is safe to run on every draw: it only fires on values no row can
    // hold (width < 20, font 0, a colour in an integer box) — the colour rows are NOT tested (0.69), so no repeat firing.
    if (!scrambled()) { repaired = true; return; }
    repaired = true;
    setListIndex(PX.book, 4); setIntegerValue(PX.width, 90); setListIndex(PX.side, 0); setListIndex(PX.thick, 2);
    ...
    setListIndex(PX.rankpos, 0); setListIndex(PX.rankscope, 0); setListIndex(PX.kinglabel, 3);
    checkBox(PX.kline, true); checkBox(PX.cw, true); checkBox(PX.pw, true); checkBox(PX.flip, true); checkBox(PX.em, true);
    checkBox(PX.extk, false); setListIndex(PX.lstyle, 1); setListIndex(PX.lpos, 0);
    checkBox(PX.roles, true); checkBox(PX.regime, true); setListIndex(PX.panelpos, 1); checkBox(PX.tapecols, true);
    checkBox(PX.lvllabels, true); setIntegerValue(PX.spywidth, 120); checkBox(PX.bands, false); setIntegerValue(PX.bandh, 3); setIntegerValue(PX.klinew, 2);
    setListIndex(PX.tnmode, 0); setListIndex(PX.tnstyle, 1); setListIndex(PX.tnwidth, 0); setListIndex(PX.ifstyle, 0); checkBox(PX.magline, false);   // (v0.75)
}
// (v0.78) ONE ROUTINE, from all three callbacks AND the top of every draw (the DS 0.16 pattern): repair a scrambled (v7-by-
// position) instance, then read — so the saved values reach the draw whenever IRT delivers them, Apply or not.
void GammaProfile::syncSettings()
{
    migrateScrambled();
    int f = getIntegerValue(PX.font);
    if (f < 1 || f > 200) return;          // values not populated yet — keep what we have, retry next draw
    readSettings(cfg); enableParameter(PX.thresh, cfg.filter == 4);
}
int GammaProfile::parmsLoad(void)  { syncSettings(); return RTX_OK; }
int GammaProfile::parmsApply(void) { syncSettings(); return RTX_OK; }
int GammaProfile::parmsUpdt(unsigned int) { syncSettings(); return RTX_OK; }
...
{
    // (v0.77) VERSION 8 — THE REGROUP. Operator 2026-09-19: "redo the settings so that they are grouped better and more orderly,
    // right now everything is too messy" -> design/gp-dialog-regroup-mockup.html, "ok build". Every row kept, none added; the
    // order follows the sections BOOK & LAYOUT · NODES · COLOURS · KING LINE · TOP NODES · IF LEVELS · READ PANEL, two per line.
    // IRT stores a saved instance's values BY POSITION, so a v7 instance reads its old values in the new slots on first load:
    // scrambled() catches it (the new SPX width slot holds the old Side, 0 or 1 — under the 20 px floor) and migrateScrambled()
    // writes the defaults once — and the defaults ARE his settings (his 09-19 dialog screenshot), so the chart looks the same.
    // No section-title rows: setLabelParameter shifted every setting below it in 0.6x (see the note above) — order only.
    // From here on, as before: a row may only be APPENDED below the last one; anything else needs another version bump.
    setParameterVersion(8);
    ...
    // BOOK & LAYOUT
    PX.book   = pc++; setListParameter   ("Book", 4, "Auto;SPX;SPY;IF;Both SPY and SPX");
    PX.side   = pc++; setListParameter   ("Side", 0, "Right;Left", 0, SL);                    // ignored when Both
    PX.width  = pc++; setIntegerParameter("SPX rail width px", 90);
    PX.spywidth=pc++; setIntegerParameter("SPY rail width px", 120, 0, SL);                    // 0 = same as the SPX width
    PX.thick  = pc++; setListParameter   ("Thickness", 2, "Auto;Thin;Medium;Thick");          // one thickness for both rails
    PX.font   = pc++; setIntegerParameter("Font size (pt)", 10, 0, SL);
    PX.tapecols=pc++; setBoolParameter   ("Tape columns (strike | %King)", true);
    ...
    PX.hideu  = pc++; setIntegerParameter("Hide nodes under %", 0, 0, SL);
    PX.rank   = pc++; setBoolParameter   ("Rank badge", true);
    PX.rankpos= pc++; setListParameter   ("Rank at", 0, "Inside;Outside", 0, SL);            // Inside flips outside on a short bar
    PX.type   = pc++; setBoolParameter   ("Node name inside", true);
    PX.rankscope=pc++;setListParameter   ("Rank for (not Top N)", 0, "Top 5;Top 3", 0, SL);
    PX.kinglabel=pc++;setListParameter   ("King name", 3, "K;KING;GPoc;GPOC;GEX;POC;GAMMA");
    PX.roles  = pc++; setBoolParameter   ("Structure labels (F/C/G/Air)", true, SL);
    // COLOURS
    PX.cpos   = pc++; setColorParameter  ("+Gamma", D_POS);
    PX.cneg   = pc++; setColorParameter  ("-Gamma", D_NEG, 0, SL);
    PX.cmid   = pc++; setColorParameter  ("Midpoint", D_MID, 0, SL);
    ...
    PX.extk   = pc++; setBoolParameter   ("Extend King line", false, SL);
    PX.lstyle = pc++; setListParameter   ("King line style", 1, "Solid;Dot;Dash");
    PX.klinew = pc++; setIntegerParameter("King line width px", 2, 0, SL);
    // TOP NODES
    PX.bands  = pc++; setBoolParameter   ("Draw top nodes", false);                             // bands or lines — off = neither
    PX.tnmode = pc++; setListParameter   ("Top nodes drawn as", 0, "Bands;Lines", 0, SL);
    PX.bandh  = pc++; setIntegerParameter("Band height (ES pts)", 3);
    PX.tnstyle= pc++; setListParameter   ("Top node line style", 1, "Solid;Dot;Dash", 0, SL);
    PX.tnwidth= pc++; setListParameter   ("Top node line width", 0, "Fixed 1 px;By %King");
    // IF LEVELS
    PX.cw     = pc++; setBoolParameter   ("Call Wall", true);
    PX.pw     = pc++; setBoolParameter   ("Put Wall", true, SL);
    PX.flip   = pc++; setBoolParameter   ("Flip", true, SL);
    PX.magline= pc++; setBoolParameter   ("Mag", false, SL);                                    // the IF Magnet as a line
    PX.em     = pc++; setBoolParameter   ("EM H/L", true, SL);
    PX.ifstyle= pc++; setListParameter   ("IF line style", 0, "Solid;Dot;Dash;None (labels only)");
    PX.lpos   = pc++; setListParameter   ("IF labels at", 0, "Left;Center;Right;Off", 0, SL);   // only levels not tagged on a node
    PX.lvllabels=pc++;setBoolParameter   ("Tag IF levels on their nodes (CW / PW / FLIP / MAG)", true);
    // READ PANEL
    PX.regime = pc++; setBoolParameter   ("Regime + read panel", true);
    PX.panelpos=pc++; setListParameter   ("Panel at", 1, "Bottom-L;Bottom-C;Bottom-R;Top-L;Top-C;Top-R", 0, SL);   // (v0.77) LAST ROW — new rows go BELOW this one
    return RTX_OK;
}
...
void GammaProfile::readSettings(Settings& S)
{
    ...
    // (v0.75) appended rows: a saved instance from 0.74 has no value for them — clamp anything a list cannot produce to its default
    S.tnmode  = gpl::listOr(getListIndex(PX.tnmode),  2, 0);
    S.tnstyle = gpl::listOr(getListIndex(PX.tnstyle), 3, 1);
    S.tnwidth = gpl::listOr(getListIndex(PX.tnwidth), 2, 0);
    S.ifstyle = gpl::listOr(getListIndex(PX.ifstyle), 4, 0);
    S.magline = isBoxChecked(PX.magline) != 0;
...
short GammaProfile::pillW(const std::string& tag, const Settings& S)
{
    ...
    return (short)gpl::pillWidth(getTextWidth(tag.c_str(), (int)tag.size()));   // (v0.76) text + bold overrun + PILL_PAD each side — the same gap left and right of the text
}
short GammaProfile::drawDepthPill(short leftX, short y, const std::string& tag, const Settings& S)
{
    ...
    textLJc((short)(leftX + gpl::PILL_PAD), y, tag.c_str(), c, S.font - 1, true);   // (v0.76) PILL_PAD in, PILL_PAD after the glyphs
    return w;
}
...
// (v0.75) a style row index (0 Solid, 1 Dot, 2 Dash) as a pen
static PEN_STYLE penOf(int st) { return st == 1 ? P_DOT : (st == 2 ? P_DASH : P_SOLID); }
...
// ---- level rail -----------------------------------------------------------
void GammaProfile::drawLevel(int lastBar, short lx, short rx, float price, COLOR col,
                             const char* label, bool extend, int style, int wpx, int lposEff, const Settings& S)
{
    PNT a; a.set(0, price); if (a.h < lx) a.h = lx;   // (v0.68) the line never runs under the SPY strip
    PNT b; b.set(lastBar, price);
    if (extend || b.h > rx) b.h = rx; // stretch into the right margin — but never under the SPX strip
    // (v0.74) through hlinePx, so a line wider than 1 px still shows its style (Windows draws wide pens solid)
    // (v0.75) each family passes ITS style (King / IF levels); 3 = "None (labels only)": no line, the label still draws
    if (gpl::styleDrawsLine(style)) hlinePx(a.v, a.h, b.h, col, penOf(style), wpx);
    ...
    if (lposEff == 3) return;
    if (lposEff == 0)      textLJ((short)(lx + 4),            (short)(y - S.font - 2), label, col, S.font, false); // left
    else if (lposEff == 1) textLJ((short)((lx + rx)/2 - 20),  (short)(y - S.font - 2), label, col, S.font, false); // center
...
void GammaProfile::render(const Settings& S, bool railOnly)
{
    ...
    if (S.bands && !bars.empty()) {
        ...
        for (size_t i = 0; i < strikes.size(); i++) {
            ...
            if (S.tnmode == 1) {   // (v0.75) "Top nodes drawn as = Lines": same nodes, same start, 1 px or By %King, the top node style
                if (s.king) continue;   // the King keeps its own King line — never drawn twice
                PNT ln; ln.set(0, s.price);
                hlinePx(ln.v, x1, bx2, pc, penOf(S.tnstyle), gpl::nodeLineWidth(s.pct, S.tnwidth == 1, S.klinew));
                continue;
            }
    ...
    if (railOnly && S.kline && kIdx >= 0) {
        ...
        hlinePx(kp.v, kx1, kx2, kc, penOf(S.lstyle), S.klinew);   // (v0.75) the King line style
    }

    // (v0.76) ROW GAPS for the tags on top of the strip: pixels from each drawn row to the nearest drawn row above it
    std::vector<int> gapAbove(strikes.size(), 10000);
    if (S.tapecols) {
        int topNg = (S.filter==0)?3 : (S.filter==1)?5 : (S.filter==2)?8 : (S.filter==3)?10 : 0;
        std::vector<short> ys(strikes.size(), (short)-32000);
        for (size_t i = 0; i < strikes.size(); i++) {
            const GStrike& s = strikes[i];
            if (gpl::nodeHidden(s.pct, s.king, S.hideu)) continue;
            bool prim = (S.filter <= 3) ? (s.rank >= 1 && s.rank <= topNg) : (S.filter == 4 ? std::fabs(s.pct) >= (float)S.thresh : true);
            if (!prim && S.below == 1) continue;
            PNT q; q.set(lastBar, s.price); ys[i] = q.v;
        }
        for (size_t i = 0; i < ys.size(); i++) { if (ys[i] == -32000) continue;
            for (size_t j = 0; j < ys.size(); j++) if (j != i && ys[j] != -32000 && ys[j] < ys[i] && ys[i] - ys[j] < gapAbove[i]) gapAbove[i] = ys[i] - ys[j]; }
    }
    short stripC1 = (S.side == 1) ? (short)(paneL + 4) : (short)(paneR - colW + 4);   // (v0.76) the strike column's left edge
    ...
    for (size_t i = 0; i < strikes.size(); i++) {
        ...
        bool tagTop = S.tapecols && gpl::tagFitsAbove(gapAbove[i], S.font);   // (v0.76) the IF / King tag goes above the strip row
        ...
        if (rl) {
            ...
            if (s.king && tagTop) rn = "";   // (v0.76) the King's name moves above its strip row; G / C / F stay in the bar
            COLOR ic = inkOn(col);
        ...
        if (wtag && tagTop) { topW = wtag; topWc = wcol; wtag = 0; }
        if (wtag) {
        ...
        if (S.tapecols) {
            ...
            if (tagTop && (kTop || topW)) {
                ...
                const char* dot = (kTop && topW) ? "\xB7" : "";
                int kw = kTop ? getTextWidth(kTop, -1) : 0, dw = dot[0] ? getTextWidth(dot, -1) : 0, ww = topW ? getTextWidth(topW, -1) : 0;
                std::string dtag = (topW && topW[0] == 'C') ? lvlDepth[1] : ((topW && topW[0] == 'P') ? lvlDepth[2] : std::string());
                short pw = dtag.empty() ? 0 : pillW(dtag, S);
                short leftLim = (S.side == 1) ? (short)(paneL + 2) : (short)(paneR - colW);
                gpl::TagRow tr = gpl::tagRow(c1, kw + dw + ww + gpl::BOLD_OVERRUN, TW.spaceW, pw, leftLim, (int)paneR - 2);
                short ty = (short)gpl::tagCentreY(p.v, S.font), tx = (short)tr.tagX;
                if (kTop) { textLJc(tx, ty, kTop, kc2, S.font - 1, true); tx = (short)(tx + kw); }
                if (dot[0]) { textLJc(tx, ty, dot, kc2, S.font - 1, true); tx = (short)(tx + dw); }
                if (topW) textLJc(tx, ty, topW, topWc, S.font - 1, true);
                if (pw) drawDepthPill((short)tr.pillX, ty, dtag, S);
            }
        }
    }
    ...
    if (S.lvllabels && has[3]) {
        ...
        if (S.tapecols) {
            // (v0.76) the tick runs from the bars across the strip; its tag sits just above the tick, on the strike column
            short fx1 = (S.side == 1) ? (short)(paneL + 2) : x1, fx2 = (S.side == 1) ? x2 : (short)(paneR - 2);
            hlinePx(fp.v, fx1, fx2, C_FLIPC, P_DASH);
            textLJc(stripC1, (short)(fp.v - (S.font - 1) / 2 - 2), fl, C_FLIPC, S.font - 1, true);
        } else {
    ...
    { COLOR kc = D_KING; if (S.kingcol != 1 && kIdx >= 0) kc = strikes[kIdx].pct < 0 ? S.cneg : S.cpos;   // (v0.64) polarity, like its bar
      ...
      if (S.kline && has[0]) drawLevel(lastBar, lvL, lvR, lvl[0], kc, "", S.extk, S.lstyle, S.klinew, S.lpos, S);   // line only; the node labels KING
    ...
    // (v0.75) the IF levels have their OWN style row; "None (labels only)" draws the label with no line (Level labels at = Off
    // then falls back to Right, or nothing at all would show)
    int ifs = S.ifstyle, lp = gpl::ifLabelPos(S.lpos, ifs);
    if (S.cw && has[1])   drawLevel(lastBar, lvL, lvR, lvl[1], C_PINK,  gpl::lineLabelWanted(S.lvllabels, onNode1, lp) ? "CALL WALL" : "", false, ifs, 1, lp, S);
    if (S.pw && has[2])   drawLevel(lastBar, lvL, lvR, lvl[2], C_PINK,  gpl::lineLabelWanted(S.lvllabels, onNode2, lp) ? "PUT WALL"  : "", false, ifs, 1, lp, S);
    if (S.flip && has[3]) drawLevel(lastBar, lvL, lvR, lvl[3], C_FLIPC, gpl::lineLabelWanted(S.lvllabels, onNode3, lp) ? "FLIP"      : "", false, ifs, 1, lp, S);
    if (S.em && has[4])   drawLevel(lastBar, lvL, lvR, lvl[4], C_CYAN,  "EM-H", false, ifs, 1, lp, S);
    if (S.em && has[5])   drawLevel(lastBar, lvL, lvR, lvl[5], C_CYAN,  "EM-L", false, ifs, 1, lp, S);
...
{
    syncSettings();   // (v0.78) the saved settings reach the draw — and a v7 instance is repaired the moment its values arrive
    load();
...
void GammaProfile::writeStatus()
{
    ...
    f << "GPRECT,paneRight=" << stPaneRaw << ",scaleL=" << stScaleL << ",scaleR=" << stScaleR << "\n"
      ...
      << ",tnmode=" << cfg.tnmode << ",tnstyle=" << cfg.tnstyle << ",tnwidth=" << cfg.tnwidth << ",ifstyle=" << cfg.ifstyle << ",magline=" << (int)cfg.magline
