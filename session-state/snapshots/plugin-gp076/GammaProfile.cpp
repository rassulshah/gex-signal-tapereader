...
static const char* GP_VERSION = "0.76";   // (v0.71) ONE version string: the factory and the status file both read it
...
int cppExtension::setup(void)
{
    ...
    PX.lstyle = pc++; setListParameter   ("King line style", 0, "Solid;Dot;Dash", 0, SL);
    ...
    PX.klinew  = pc++; setIntegerParameter("King line width px", 2);                          // (v0.64) APPENDED
    // (v0.75) APPENDED — operator 2026-09-19: "a separate line style for the king than the top 5 nodes", "an option to make the line
    // thicker based on the %king", "a separate line style for levels we get from IF", "the ability to only have labels without any lines"
    PX.tnmode  = pc++; setListParameter   ("Top nodes drawn as", 0, "Bands;Lines");
    ...
    PX.ifstyle = pc++; setListParameter   ("IF level line style (CW / PW / Flip / Mag / EM)", 0, "Solid;Dot;Dash;None (labels only)", 0, SL);
    PX.magline = pc++; setBoolParameter   ("Mag line (IF Magnet)", false);                         // (v0.75) APPENDED LAST — new rows go BELOW this one
    return RTX_OK;
}
