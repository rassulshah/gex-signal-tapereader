...
## 📦📦 THE ONE-FILE RULE — user-mandated 2026-08-15, RESTATED VERBATIM 2026-08-27

> **"you are supposed to just give me an install file."**

**A build is delivered as EXACTLY ONE `.bat`.** He downloads it and double-clicks it. Not a zip plus
an applier. Not "primary and fallback, here are both". Not the installer plus a render plus a
screenshot. **ONE FILE**, plus the Tampermonkey links pasted as text.

⚠ **HOW THIS BROKE, so it is not repeated:** `tools/build-installer.py` printed
a two-file delivery banner naming `gexdrop.zip + applygex.bat`, this file called that pair "the
primary delivery", and `skills/gex/SKILL.md` said "ship ONE self-contained installer". Three
documents, two answers. A context followed the tool's banner over the operator's standing rule.
**All three now say ONE, `test_delivery.js` fails the build if any of them stops saying it, and the
builder prints the single filename it wants sent.**

⚠ The zip+applier pair is still BUILT and still works. Send it only if he reports the `.bat` failed.
...
## Delivery landmines (2026-08-25, all hit in one day)
- **Downloads strip DASHES from filenames** ("gex-v14.2-drop.zip" arrived as "gexv14.2drop.zip"),
  so delivery filenames carry no dashes and no dots except the extension, and the applier finds its
  zip by WILDCARD (beside itself, repo mockups, Downloads).
- **A .bat written with LF line endings closes instantly on double-click.** Every hand-made or
  generated .bat must be CRLF. The builder asserts this.
- **certutil -f -decode is an AV target** (same family as the PowerShell IDP.HELU.PSE88 rule), so
  a zip + `tar -xf` applier pair exists as the FALLBACK for the day the self-extractor fails again.
  ⚠⚠ **IT IS NOT THE DELIVERY.** This entry used to call the pair "the primary delivery" while the
  skill said "ship ONE self-contained installer" — and on 2026-08-27 a context resolved that
  contradiction the wrong way and sent him three attachments. See THE ONE-FILE RULE below.
