...
- **certutil -f -decode is an AV target** (same family as the PowerShell IDP.HELU.PSE88 rule).
  The primary delivery is now zip + `tar -xf` (bsdtar reads zip natively on Win10+) — no certutil,
  no base64, no self-extraction. Generated by tools/build-installer.py alongside the fallback.
