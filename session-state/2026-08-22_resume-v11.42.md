# RESUME NOTE — 2026-08-22 — v11.42 / companion v1.7
...
prints the payload's structure. ⚠ **OPEN THREAD: run `ifShape()` and check whether their zero gamma is
nested. If it is, take theirs and delete the derived flip.**
...
- **Level depth** = GEX ∩ DEX overlap, tiered ◆◆ / ◆. ⚠ The books sit on DIFFERENT strike grids, so
  depth aggregates across a ZONE (max, not sum) — taking the nearest single key read one book and scored
  the other zero, making every level look gamma-only.
...
- **Two install failures look identical from the user's side.** One call separates them: fetch the raw
  GitHub URL and read `@version`. GitHub old → the installer did not push (my problem). GitHub new,
  browser old → Tampermonkey cache, or the page simply needs a reload.
