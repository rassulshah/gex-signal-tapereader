...
⚠⚠ **AND THE CORRECTION FOUND THE REAL BUG, WHICH IS BIGGER THAN THE ONE I MIS-REPORTED.** The fix
works; the BUDGET does not. `LS_BUDGET_KB['gpts_recorder_v7']` is **3600 KB** against a **~6 MB**
session, so the shedder trims today oldest-first exactly as designed and the export — which runs at
the close — keeps only what survived. Measured on `data/2026-08-31.json`: snapshots span
**08:30-15:00**, feature records span **13:36-15:00**, the newest 29 bars of 131.
**The silent write failure became silent SHEDDING**, and the hours it discards are the ones the
models live in — the ⓪a NOT-IN call fires at a median 08:40, GREEN/RED at 09:03.

⚠ `LS_HEALTH` counts `shed` and `quotaHits` and **`buildDayExport` exports none of it**, so the day
file cannot distinguish "a quiet morning" from "the morning was thrown away". **A fix that reports
its own failures must put that report where the failures are read** — which for this project is the
day file, not a console hook.
