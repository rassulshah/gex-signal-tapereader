...
**v11.4 (IRT FlexLevels export — gear toggle, 1/3/5/15m cadence, folder handle, EPU26 conversion; user setup pending: pick folder + add FlexLevels indicator in IRT) + v11.3.2 (PB Entry second line) + v11.3.3 (FEED FRESHNESS GUARD — historical payloads can no longer replace the live book; 08-19 bars ~13:40–14:40 CT are poisoned with px≈777.66 and must be quarantined by the nightly) ALSO SHIPPED. v11.3 + v11.3.1 SHIPPED 2026-08-19 (12:30 / 13:10 CT; 11.3.1 = pullback context per record: pb depth/pace, SMA-50 frame, node fresh/rolled, stacked nodes with strongest/deepest) — PB ENTRY line under Next Stop (user-directed, enrolled: pbEntry/pbEntry.60, sequenced touch-then-deflect outcome, rules 58). v11.2 SHIPPED 11:30 CT — $K-ANCHORED LADDER READER (user's principle) + Trinity SPY/QQQ/SPXW/VIX ladders read and recorded (`tri`); see CHANGELOG. v11.1.3 + 11.1.3.1 HOTFIXES earlier the same morning (live, 10:20 / 10:45 CT; 11.1.3.1 adds `bookKing` = the $K cell wherever it sits, recorded as snap `bk`):** tape sync red all morning with EXPIRATIONS on → three Layer-0 faults fixed
