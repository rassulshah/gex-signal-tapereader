...
console.log('[GPTS] v10.5 part1 loaded');
...
function injectSliderCss(){
  ...
  st.textContent =
    ...
    // #7 S/R vertical scroll box + King-path horizontal scroll: thin dark theme.
    ...
    '#gpts-panel #gpts-kingpath::-webkit-scrollbar{ height:6px; }'+
    '#gpts-panel #gpts-kingpath::-webkit-scrollbar-thumb{ background:#3a4150; border-radius:4px; }'+
    '#gpts-panel #gpts-kingpath::-webkit-scrollbar-track{ background:#0f131b; }';
...
function structuralReadHtml(){
  ...
  // #2 TARGET BADGES: distance/likelihood is now encoded as COLOR, not a text
  // row. T1 keeps GEOMETRY color (green if above price = bullish, red if below =
  // bearish) so the trader reads direction off T1. T2's color encodes REACH
  // (green=in reach, amber=stretch, red=far); direction is inferred from T1.
  // Full distance sentence lives in each badge's tooltip.
  var proxS = adaptiveProxStrikes('SPY');
  function reachInfo(k){
    if(pxT==null||k==null) return { col:PAL.sub, word:'\u2013' };
    var aStr=Math.abs(k-pxT);
    if(aStr<=proxS)        return { col:PAL.longAccent, word:'in reach' };
    if(aStr<=proxS*1.8)    return { col:PAL.amber,      word:'stretch' };
    return { col:PAL.shortAccent, word:'far' };
  }
  function geomCol(k){ if(pxT==null||k==null) return PAL.sub; return (k>pxT)?PAL.longAccent:PAL.shortAccent; }
  function tgtTip(k, tag, isT1){
    if(pxT==null||k==null) return '';
    var pts=(k-pxT), aStr=Math.abs(k-pxT), ri=reachInfo(k);
    var dir=(pts>0?'above (bullish)':'below (bearish)');
    var base=tag+' '+fmtNum(k)+' is '+(pts>0?'+':'')+pts.toFixed(1)+' pts ('+aStr.toFixed(0)+' strike'+(aStr>=2?'s':'')+') '+dir+' price. At the recent 3m pace (~'+proxS+' strikes/pullback) it is '+ri.word+'.';
    return (isT1
      ? (base+' Color = direction (green=above/bullish, red=below/bearish).')
      : (base+' Color = reach: green=in reach, yellow=stretch, red=far. Direction is the same side as T1.')
    ).replace(/"/g,'');
  }
  var c1=geomCol(meta.t1), c2=reachInfo(meta.t2).col;
  var hdrRight = trendBadgeHtml()+
      (meta.t1Label?'<span title="'+tgtTip(meta.t1,'T1',true)+'" style="color:'+c1+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+c1+';border-radius:20px">'+meta.t1Label+'</span>':'')+
      (meta.t2Label?'<span title="'+tgtTip(meta.t2,'T2',false)+'" style="color:'+c2+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+c2+';border-radius:20px">'+meta.t2Label+'</span>':'');
  var html = sectionHdrRight('Trend', hdrRight, 'TREND: the coordinated one-line take on structure right now \u2014 what price is doing relative to the nearest node, the trend (Up/Dn/Side/NA), and the two targets T1/T2. T1 color = direction (green above / red below). T2 color = reach (green in reach, yellow stretch, red far); its direction matches T1. Hover a target for its exact distance.');
...
function kingBlock(){
  ...
  // ---- Distance badge: 'at price' when 0, else 'up N above' / 'dn N below' ----
  var distHtml='';
  if(px!=null && kingK!=null){
    var d=Math.abs(kingK-px), dr=Math.round(d);
    if(dr===0){
      distHtml='<span title="King sits at the current price \u2014 settlement gravity right here." style="color:'+PAL.sub+';font-weight:700;font-size:10px;padding:1px 6px;border:1px solid '+PAL.line+';border-radius:20px">at price</span>';
    } else {
      var above = kingK>px;
      var sideC = above?PAL.shortAccent:PAL.longAccent;  // above=resistance(red), below=support(green)
      var arr = above?'\u2191':'\u2193';
      distHtml='<span title="How far the King strike is from price ('+dr+' strike'+(dr>1?'s':'')+' '+(above?'above':'below')+')." style="color:'+sideC+';font-weight:700;font-size:10px;padding:1px 6px;border:1px solid '+sideC+';border-radius:20px">'+arr+' '+dr+' '+(above?'above':'below')+'</span>';
    }
  }

  // ---- Net session drift: first recorded King -> current ----
  var firstK = mv.length ? mv[0].k : kingK;
  var net = (firstK!=null && kingK!=null) ? (kingK-firstK) : 0;
  var netC = net>0?PAL.longAccent:(net<0?PAL.shortAccent:PAL.sub);
  var netArr = net>0?'\u2191':(net<0?'\u2193':'\u2192');
  var netHtml='';
  if(mv.length>=1){
    netHtml='<span title="Net session drift of the King: '+fmtNum(firstK)+' \u2192 '+fmtNum(kingK)+' ('+(net>0?'+':'')+net+' strikes). Up = dealers migrating their settlement target higher (bullish), down = lower (bearish)." style="color:'+netC+';font-weight:700;font-size:10px;padding:1px 6px;border:1px solid '+netC+';border-radius:20px">'+netArr+Math.abs(net)+' net</span>';
  }

  // ---- Header ----
  var hdrRight =
    '<span title="Current King strike (largest absolute dealer exposure)." style="color:'+PAL.gold+';font-weight:800;font-size:10px;padding:1px 7px;border:1px solid '+PAL.gold+';border-radius:20px">'+(kingK!=null?fmtNum(kingK):'\u2013')+'</span>'+
    distHtml + netHtml +
    '<span title="Confirmed King rolls today (persisted across reloads)." style="color:'+PAL.sub+';font-weight:700;font-size:10px;padding:1px 6px;border:1px solid '+PAL.line+';border-radius:20px">'+moveCount+'</span>';
  var html=sectionHdrRight('\uD83D\uDC51 King', hdrRight, 'KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s center of gravity and settlement magnet. Header: current King, distance from price, net session drift, and today\u2019s roll count. Below: the roll path, oldest \u2192 newest (current on the right). Hover any chip for its time and roll direction.');
  if(kingK==null){
  ...
  // ---- Roll path: OLDEST \u2192 NEWEST (current on the RIGHT). Auto-scrolls to the
  // right end on render so the latest King is always in view. Chips show the
  // PRICE only; time + roll direction live in each chip's hover tooltip.
  var pathHtml='';
  if(mv.length){
    // Build oldest->newest, collapsing consecutive same-strike entries.
    var disp=[];
    for(var j=0;j<mv.length;j++){
      var m=mv[j];
      if(disp.length && Math.abs(disp[disp.length-1].k-m.k)<0.001){ disp[disp.length-1]=m; continue; } // keep latest ts of a repeat
      disp.push(m);
    }
    var chips=[];
    for(var c=0;c<disp.length;c++){
      var mm=disp[c];
      var isNow=(c===disp.length-1);
      var cc = isNow?PAL.gold : (mm.dir>0?PAL.longAccent:(mm.dir<0?PAL.shortAccent:PAL.sub));
      var dirWord = mm.dir>0?'rolled up (bullish)':(mm.dir<0?'rolled down (bearish)':'first seen');
      var stamp = isNow ? ('current \u00b7 pinned '+pinnedTxt) : fmtClock(mm.t);
      var chipTip = ('King '+fmtNum(mm.k)+' \u2014 '+stamp+' \u00b7 '+dirWord+'.').replace(/"/g,'');
      var bg = isNow?'background:rgba(227,195,65,0.12);':'';
      chips.push('<span title="'+chipTip+'" style="flex:0 0 auto;color:'+cc+';font-weight:'+(isNow?'800':'700')+';border:1px solid '+cc+';border-radius:4px;padding:2px 7px;'+bg+'">'+fmtNum(mm.k)+'</span>');
    }
    var arrowJoin='<span style="flex:0 0 auto;color:'+PAL.line+';font-size:9px;padding:0 1px">\u2192</span>';
  ...
  var pathTip=('Today\u2019s King roll path, oldest \u2192 newest (current on the right, gold). Consecutive repeats collapsed. Hover any chip for its time and roll direction (up=bullish, down=bearish). '+moveCount+' confirmed roll'+(moveCount===1?'':'s')+' today.').replace(/"/g,'');
  html+='<div style="padding:5px 8px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;margin:2px 0">'+
    '<div style="color:'+PAL.sub+';font-size:8px;font-weight:700;letter-spacing:.4px;margin-bottom:4px">KING PATH \u00b7 oldest \u2192 current</div>'+
    '<div id="gpts-kingpath" title="'+pathTip+'" style="display:flex;gap:0;align-items:center;overflow-x:auto;white-space:nowrap;padding-bottom:2px;font-size:11px;font-variant-numeric:tabular-nums">'+pathHtml+'</div>'+
  '</div>';
  return html;
}
...
function accumBlock(){
  ...
  var px = STATE.SPY.price;
  // #5 Removed the in-play header badge (replaced elsewhere). Header is now just
  // the S/R label; the ladder itself shows what's in play via its border colors.
  var html=sectionHdrRight('S/R', '', 'S/R (Support / Resistance monitor): only the nodes that matter right now \u2014 those actively Accumulating (building) or Dissipating (fading), plus the King. Each node\u2019s %King is tracked over time. Green = building support (below price), yellow = building resistance (above), red = dissipating. PROJ flags the single strongest-building floor & ceiling across a wide band. Hover any node for its timestamped growth.');

  // ===== MERGED BIAS BLOCK (NET board tilt + LOCAL nearest-levels read) =====
  // These are the same support-vs-resistance question at two zoom levels:
  //   BOARD = netPositioning() summing all accumulation above vs below price;
  //   LOCAL = relativeRead() comparing the nearest building support vs the
  //           nearest building resistance within adaptive proximity.
  // One block, a balance bar for the board tilt, then two scope tiles.
  var net = netPositioning('SPY');
  var rel = relativeRead('SPY');
  if((net && net.bias!=='balanced') || (rel && (rel.sup || rel.res))){
    // ---- Board tilt (headline + bar) ----
    var haveNet = !!(net && net.bias!=='balanced');
    var netDir = haveNet ? net.dir : 0;
    var netCol = netDir>0?PAL.longAccent:(netDir<0?PAL.shortAccent:PAL.sub);
    var below = (net && typeof net.below==='number') ? net.below : 0;
    var above = (net && typeof net.above==='number') ? net.above : 0;
    var totNB = below+above;
    var belowPct = totNB>0 ? Math.round(below/totNB*100) : 50;
    var abovePct = 100-belowPct;
    var headline = haveNet
      ? ((netDir>0?'Support-heavy ':'Resistance-heavy ')+net.ratio.toFixed(1)+'\u00d7')
      : 'Balanced';
    var boardBias = netDir>0?'dip-buy bias':(netDir<0?'fade/short bias':'no board tilt');
    var boardTip = (haveNet
      ? ('BOARD tilt \u2014 summing accumulation ABOVE vs BELOW price (build-rate weighted, nearer nodes heavier): dealers are building '+net.ratio.toFixed(1)+'\u00d7 more '+(netDir>0?'below (downside defended \u2192 dips favored)':'above (rallies capped \u2192 fades favored)')+'.'+(net.decisive?' Decisive tilt.':' Mild tilt.'))
      : 'BOARD tilt \u2014 accumulation is roughly balanced above vs below price; no directional lean yet.').replace(/"/g,'');

    // ---- Local read (relabeled: Support edge / Resistance edge / Neutral) ----
    var locVerd='Neutral', locCol=PAL.sub, locC='neutral';
    if(rel){
      if(rel.cls==='long'){ locVerd='Support edge'; locCol=PAL.longAccent; locC='long'; }
      else if(rel.cls==='trap'){ locVerd='Resistance edge'; locCol=PAL.shortAccent; locC='trap'; }
      else { locVerd='Neutral'; locCol=PAL.sub; locC='neutral'; }
    }
    var win=(HIST_MAX*3);
    var supTx = (rel&&rel.sup) ? (fmtNum(rel.sup.k)+' '+(rel.supRate>0?'+':'')+Math.round(rel.supRate)) : 'none';
    var resTx = (rel&&rel.res) ? (fmtNum(rel.res.k)+' '+(rel.resRate>0?'+':'')+Math.round(rel.resRate)) : 'none';
    var locTip = (rel
      ? (locVerd+' \u2014 nearest building support below ('+supTx+'/'+win+'m) vs nearest building resistance above ('+resTx+'/'+win+'m) within ~'+rel.prox+' strikes. '+(locC==='long'?'Support is building faster than overhead \u2192 a dip here has room to work.':locC==='trap'?'Overhead resistance is loading harder than support \u2192 a bounce likely caps into it.':'Neither nearby side is building decisively \u2014 no immediate edge; wait for a tilt.'))
      : 'No nearby building levels within proximity yet.').replace(/"/g,'');

    // ---- Render: header + balance bar + BOARD / LOCAL tiles ----
    html+=sectionHdrRight('\u2696 Bias',
      '<span title="'+boardTip+'" style="color:'+netCol+';font-weight:800;font-size:10px;padding:1px 7px;border:1px solid '+netCol+';border-radius:20px">'+headline+'</span>',
      'BIAS: support vs resistance at two zoom levels. BOARD = the whole board\u2019s accumulation tilt (bias for the day). LOCAL = the nearest building support vs resistance right now (immediate edge). The bar shows how exposure splits below (support) vs above (resistance) price.');
    html+='<div style="background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;padding:7px 9px;margin:2px 0">'+
      // balance bar
      '<div title="'+boardTip+'" style="display:flex;height:8px;border-radius:4px;overflow:hidden;margin-bottom:5px;background:'+PAL.bg+'">'+
        '<div style="width:'+belowPct+'%;background:'+PAL.longAccent+'"></div>'+
        '<div style="width:'+abovePct+'%;background:'+PAL.shortAccent+'"></div>'+
      '</div>'+
      '<div style="display:flex;justify-content:space-between;font-size:8px;font-weight:700;margin-bottom:7px">'+
        '<span style="color:'+PAL.longAccent+'">\u25c0 support '+belowPct+'%</span>'+
        '<span style="color:'+PAL.shortAccent+'">resistance '+abovePct+'% \u25b6</span>'+
      '</div>'+
      // two scope tiles
      '<div style="display:flex;gap:6px">'+
        '<div title="'+boardTip+'" style="flex:1;background:'+PAL.bg+';border:1px solid '+PAL.line+';border-radius:6px;padding:4px 7px;min-width:0">'+
          '<div style="color:'+PAL.sub+';font-size:8px;font-weight:700;letter-spacing:.3px">BOARD</div>'+
          '<div style="color:'+netCol+';font-size:11px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+boardBias+'</div>'+
          '<div style="color:'+PAL.sub+';font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+headline+'</div>'+
        '</div>'+
        '<div title="'+locTip+'" style="flex:1;background:'+PAL.bg+';border:1px solid '+PAL.line+';border-radius:6px;padding:4px 7px;min-width:0">'+
          '<div style="color:'+PAL.sub+';font-size:8px;font-weight:700;letter-spacing:.3px">LOCAL \u00b7 ~'+(rel?rel.prox:'?')+' str</div>'+
          '<div style="color:'+locCol+';font-size:11px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+locVerd+'</div>'+
          '<div style="color:'+PAL.sub+';font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">sup '+supTx+' \u00b7 res '+resTx+' /'+win+'m</div>'+
        '</div>'+
      '</div>'+
    '</div>';
  }
...
function render(){
  if(!elBody) return;
  var html='';
  // #6 Section order: Trend → S/R → King → BO.
  html+=structuralReadHtml();   // Trend (targets T1/T2, color-coded)
  html+=sep();
  html+=accumBlock();           // S/R (Proj callout + Bias + scrollable ladder)
  html+=sep();
  ...
  if(gEl){
    ...
    if(g){
      ...
      gEl.textContent=g.letter+'\u00b7'+(g.dir==='Long'?'L':'S');
  ...
  var kp=document.getElementById('gpts-kingpath');
  if(kp){ kp.scrollLeft = kp.scrollWidth; }
}
