...
function findTapeTable(){
  // The heatmap sidebar table has a header cell reading "Strike". Prefer a real
  // <table>; fall back to any container that has a "Strike" label + a $K cell.
  ...
  for(var j=0;j<all.length;j++){
    ...
    if(t.indexOf('Strike')!==-1 && /\$[\d,]+K/.test(t) && all[j].querySelectorAll('*').length<400) return all[j];
  }
  return null;
}
function readTapeFromDOM(sym){
  ...
  // Build rows: prefer <tr>, else group leaf cells by their vertical position.
  ...
  if(trs.length){
    for(var i=0;i<trs.length;i++){
      ...
      // First data column = cells[1]: read its unsigned %King strength.
      // (Skip a leading signed change value if the cell holds both.)
