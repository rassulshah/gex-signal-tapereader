...
console.log('[GPTS] v10.28 part1 loaded');
...
function nodeRoleBadge(L){
  ...
  if(L.isKing){
    ...
    label=(L.rugType==='Reverse-Rug')?'RRug-Fl':'Rug-Ce'; col='#b58bff';
    tip=((L.rugType==='Reverse-Rug')?'Reverse-Rug floor':'Rug ceiling')+' ('+(L.rugType||'Rug')+') \u2014 positive-gamma cap over a negative-gamma floor.';
  } else if(L.isRugFloor){
    label=(L.rugType==='Reverse-Rug')?'RRug-Ce':'Rug-Fl'; col='#b58bff';
    tip=((L.rugType==='Reverse-Rug')?'Reverse-Rug ceiling':'Rug floor')+' ('+(L.rugType||'Rug')+') \u2014 negative-gamma target below the positive-gamma cap.';
  } else if(L.isStack){
    label='2Stk'; col=PAL.longAccent;
    tip='Double-Stack \u2014 adjacent stacked nodes forming a strong-BOUNCE shelf; price tends to bounce cleanly off it.';
  } else if(L.isCluster){
    label='Clu'; col=PAL.blue;
...
function nodeMapBlock(){
  ...
  if(stk && stk.ok){
    stk.stacks.forEach(function(s){
      ...
      html+='<div title="Double-Stack ('+s.n+' adjacent significant nodes within '+STACK_GAP+' strike) \u2014 a strong-bounce shelf; price tends to bounce cleanly off it (fade into it)." style="font-size:9.5px;color:'+PAL.longAccent+';font-weight:800;margin:0 2px 4px">\u2b1b\u2b1b 2Stk '+span+' \u00b7 bounce shelf</div>';
    });
  }
  ...
  if(cl && cl.ok){
    cl.regions.forEach(function(r){
      html+='<div title="Cluster ('+r.n+' large nodes grouped within '+CLUSTER_BAND+' strikes) \u2014 price tends to PIN / CHOP through this region. Poor R:R in the middle; trade the edges." style="font-size:9.5px;color:'+PAL.blue+';font-weight:800;margin:0 2px 4px">\u25a6 Clu '+fmtNum(r.lo)+'\u2013'+fmtNum(r.hi)+' \u00b7 pin/chop</div>';
    });
  }
