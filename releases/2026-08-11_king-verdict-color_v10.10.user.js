...
console.log('[GPTS] v10.10 part1 loaded');
...
function kingSparkline(mv, kingK, px, now, sess, verdictCol){
  ...
  // (v10.10) Line color is the KING VERDICT (green bull / red bear / yellow flat),
  // passed in \u2014 not just the raw slope \u2014 so the chart color encodes the read.
  var lineCol = verdictCol || (netDir>0?PAL.longAccent:(netDir<0?PAL.shortAccent:PAL.sub));
  var svg='<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="xMidYMid meet" style="display:block;width:100%;height:auto">';
  // faint verdict-colored fill under the staircase (baseline at chart bottom)
  var baseY=(H-padB).toFixed(1);
  var fillD=d+' L '+X(xMax).toFixed(1)+' '+baseY+' L '+X(pts[0].t).toFixed(1)+' '+baseY+' Z';
  svg+='<path d="'+fillD+'" fill="'+lineCol+'" opacity="0.08" stroke="none"/>';
