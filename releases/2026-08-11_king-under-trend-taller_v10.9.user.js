...
console.log('[GPTS] v10.9 part1 loaded');
...
function render(){
  if(!elBody) return;
  var html='';
  // Section order (v10.9): Trend → King → S/R → BO. King sits right under Trend
  // so the directional read (trend + King drift) is grouped at the top.
  html+=structuralReadHtml();   // Trend (targets T1/T2, color-coded)
  html+=sep();
  html+=kingBlock();            // King tracker (sparkline)
  html+=sep();
