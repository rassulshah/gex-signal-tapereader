...
console.log('[GPTS] v10.52 part1 loaded');
...
function pipeStages(){
  ...
  var svLabel=(sv.code==='saved')?'saved':((sv.code==='download')?'dl':'none');
  var svState=(sv.code==='saved')?'green':((sv.code==='download')?'amber':'red');
...
function feedStatusHtml(){
  ...
  return '<div title="'+footTip+'" style="display:flex;align-items:center;gap:5px;color:'+PAL.sub+';font-size:8px;letter-spacing:0.2px;white-space:nowrap;overflow:hidden">'+
    strip+
    ctrls+
    '<span style="margin-left:auto">v10.52</span>'+
  '</div>';
}
...
function analysisBlock(){
  ...
  // (v10.52 D) when the review arrived by itself from GitHub, say which day's file it is.
  var __pipeRev=null; try{ var __P=pipeLoad(); if(__P && __P.review==='yes') __pipeRev=__P.reviewDay||null; }catch(ePR){}
  var srcLabel = loaded? ('\uD83D\uDCC2 loaded '+(LOADED_DAY.date||'file'))
                       : (R?('\u25f7 review '+(__pipeRev?(__pipeRev+' \u2014 read back from GitHub'):'loaded')):'\u25f7 live from tape data');
