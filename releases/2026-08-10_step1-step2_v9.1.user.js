...
function accumulationStateFor(sym, k){
  var r = rawAccumNode(sym, k);
  if(!r || !r.seq || r.seq.length<2){ return { label:'Steady', delta:0, seq:[], available:false }; }
  var delta = (typeof r.delta==='number') ? r.delta : ((r.seq[r.seq.length-1]||0) - (r.seq[0]||0));
  var label = 'Steady';
  if(delta >= ACC_NET) label='Building';
  else if(delta <= -ACC_NET) label='Fading';
  return { label:label, delta:delta, seq:r.seq||[], available:true, pct:(typeof r.last==='number'?r.last:r.seq[r.seq.length-1]), pos:r.pos, absSeq:r.absSeq||[] };
}
function classifyNodeRole(sym, wall, px, meta){
  if(!wall) return 'Node';
  if(meta && meta.t2!=null && Math.abs(wall.k-meta.t2)<0.001) return 'King';
  ...
  for(var i=0;i<walls.length;i++){
    ...
    if(Math.abs(w.k-wall.k)<=1.0) neighbors++;
  }
  if(neighbors>=1) return 'Cluster';
  if(px!=null){
...
function futureStructureSummary(sym){
  ...
  for(var i=0;i<walls.length;i++){
    ...
    var row={
      ...
      pct:w.pct,
      pos:w.pos,
      role:classifyNodeRole(sym, w, px, meta),
      side:(px!=null && w.k>px)?'above':'below',
      state:accumulationStateFor(sym, w.k),
      abs:w.abs
    };
  ...
  var inPlay = null;
  if(dir==='long'){
    for(var x=0;x<above.length;x++){ if(above[x].state.label==='Building'){ inPlay=above[x]; break; } }
    if(!inPlay) inPlay = nearestAbove || nearestBelow;
  } else if(dir==='short'){
    for(var y=0;y<below.length;y++){ if(below[y].state.label==='Building'){ inPlay=below[y]; break; } }
    if(!inPlay) inPlay = nearestBelow || nearestAbove;
  } else {
    inPlay = nearestAbove || nearestBelow;
  }
...
function structuralReadHtml(){
  ...
  var anc = fs.current;
  var html = sectionHdr('Read');
  html += '<div style="display:flex;justify-content:flex-end;align-items:center;gap:6px;padding:5px 9px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;margin:2px 0">'+
      trendBadgeHtml()+
...
function accumBlock(){
  var fs = futureStructureSummary('SPY');
  var rowsAbove = fs.above.filter(function(r){ return r.state.label==='Building' || r.state.label==='Steady' || r.state.label==='Fading'; }).slice(0,2);
  var rowsBelow = fs.below.filter(function(r){ return r.state.label==='Building' || r.state.label==='Steady' || r.state.label==='Fading'; }).slice(0,2);
  var inPlay = fs.inPlay;
  var px = STATE.SPY.price;
  var html=sectionHdr('ACM');
  var inPlayText='No node in play';
  var inPlayColor=PAL.sub;
  if(inPlay){
    var abbr = inPlay.role==='Gatekeeper' ? 'GK' : (inPlay.role==='King' ? 'K' : inPlay.role);
    inPlayText = abbr+' '+fmtNum(inPlay.k)+' In Play';
    inPlayColor = inPlay.side==='above' ? PAL.shortAccent : PAL.longAccent;
  }
  var inPlayState = inPlay ? inPlay.state.label : null;
  html += '<div style="display:flex;justify-content:flex-end;align-items:center;gap:6px;padding:5px 9px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;margin:2px 0">'+
      '<span title="The node price is interacting with or approaching that is most likely to matter next." style="color:'+inPlayColor+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+inPlayColor+';border-radius:20px">'+inPlayText+'</span>'+
      (inPlayState?'<span title="Current accumulation state for the in-play node." style="color:'+(inPlayState==='Building'?PAL.longAccent:(inPlayState==='Fading'?PAL.shortAccent:PAL.blue))+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+(inPlayState==='Building'?PAL.longAccent:(inPlayState==='Fading'?PAL.shortAccent:PAL.blue))+';border-radius:20px">'+inPlayState+'</span>':'')+
  '</div>';
  ...
  function rowHtml(r){
    ...
    var label = (r.side==='below' && r.role==='Cluster') ? 'Cluster' : r.role;
    var tip = ('Node '+fmtNum(r.k)+' is '+(r.side==='above'?'above':'below')+' price. Classification: '+r.role+'. Strength '+r.pct+'% of King. Accumulation path '+(r.seq&&r.seq.length?accTrajHtml(r.seq):'n/a')+'. State '+r.state.label+'.').replace(/"/g,'');
    var stateTip;
    if(r.state.label==='Building') stateTip=('Building: node strength is increasing over time. Recent path '+accTrajHtml(r.seq||[])+' makes this level more important as price approaches it.').replace(/"/g,'');
    else if(r.state.label==='Fading') stateTip=('Fading: node strength is decreasing over time. Recent path '+accTrajHtml(r.seq||[])+' suggests the level is losing influence.').replace(/"/g,'');
    else stateTip=('Steady: node strength is holding near the same range over time. Recent path '+accTrajHtml(r.seq||[])+' keeps the level relevant without strong change.').replace(/"/g,'');
    return '<div title="'+tip+'" style="display:grid;grid-template-columns:48px 1fr auto;gap:8px;align-items:center;padding:3px 7px;margin:2px 0;background:'+PAL.card+';border:1px solid '+PAL.line+';border-left:3px solid '+left+';border-radius:8px">'+
      '<span style="color:'+PAL.ink+';font-weight:800;font-size:11px">'+fmtNum(r.k)+'</span>'+
      '<span style="color:'+PAL.ink+';font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+label+' · '+r.pct+'%</span>'+
      '<span title="'+stateTip+'" style="color:'+stateColor(r.state.label)+';font-size:10px;font-weight:700;padding:1px 6px;border:1px solid '+stateColor(r.state.label)+';border-radius:20px">'+r.state.label+'</span>'+
    '</div>';
  }
  if(rowsAbove.length){
    html+='<div style="color:'+PAL.sub+';font-size:9px;font-weight:700;letter-spacing:0.4px;padding:1px 4px 0 4px">RESISTANCE ABOVE PRICE</div>';
    rowsAbove.forEach(function(r){ html+=rowHtml(r); });
  }
  if(px!=null){
    html+='<div title="Current SPY price divider." style="display:flex;justify-content:center;align-items:center;height:24px;margin:3px 0;background:'+PAL.card+';border:1px dashed '+PAL.line+';border-radius:8px;color:'+PAL.ink+';font-size:12px;font-weight:800">SPY '+fmtNum(px)+'</div>';
  }
  if(rowsBelow.length){
    html+='<div style="color:'+PAL.sub+';font-size:9px;font-weight:700;letter-spacing:0.4px;padding:1px 4px 0 4px">SUPPORT BELOW PRICE</div>';
