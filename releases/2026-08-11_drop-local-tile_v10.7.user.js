...
console.log('[GPTS] v10.7 part1 loaded');
...
function accumBlock(){
  ...
  // (LOCAL/relativeRead tile removed in v10.7 \u2014 redundant with the node rows +
  //  Trend sentence; relativeRead still runs inside the trap alert.)
  ...
  var html=sectionHdrRight('S/R Bias', srBiasHdrRight, 'S/R BIAS: support vs resistance. The nodes list shows only the strikes that matter right now (Accumulating / Dissipating, plus the King); PROJ flags the strongest forming floor & ceiling. The header badge + balance bar are the BOARD tilt (the day bias): green = accumulation building below price (support), red = above (resistance). Green = building support, yellow = building resistance, red = dissipating in the node rows.');

  // (v10.7) LOCAL tile removed: the nearest building support-vs-resistance read
  // it summarized is already visible in the two S/R node rows straddling the
  // price divider and stated in the Trend sentence. The BOARD tilt is now shown
  // ONLY as the header badge + this balance bar (its own tile was redundant with
  // the bar). relativeRead() is kept alive \u2014 it still feeds the trap alert.
  if(net && net.bias!=='balanced'){
    html+='<div title="'+boardTip+'" style="background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;padding:7px 9px;margin:2px 0">'+
      // balance bar = the BOARD tilt drawn visually (green support below vs red resistance above)
