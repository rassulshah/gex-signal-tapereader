...
var __FEED_PROBE_DONE = {};
function __feedProbe(sym, feed, j){
  try{
    var key=sym+':'+feed;
    var lv=j.levels||[]; if(!lv.length) return;
    var last=lv[lv.length-1];
    var nodes=last.l||[]; if(!nodes.length) return;
    var n0=nodes[0];
    __PROBE_RESULT.feed={ key:key, nodeKeys:Object.keys(n0), sample:n0,
      volKeys:Object.keys(n0).filter(function(kk){ return /vol|buy|sell|bid|ask|oi|flow|delta|cvd|qty|size/i.test(kk); }) };
    if(__FEED_PROBE_DONE[key]) return;
    __FEED_PROBE_DONE[key]=true;
    console.log('[FEED-PROBE] '+key+' | top-level j keys: '+Object.keys(j).join(','));
    console.log('[FEED-PROBE] '+key+' | last-slice keys: '+Object.keys(last).join(','));
    console.log('[FEED-PROBE] '+key+' | NODE keys: '+Object.keys(n0).join(','));
    console.log('[FEED-PROBE] '+key+' | NODE sample: '+JSON.stringify(n0));
    // probe for any volume / buy-sell / flow style fields anywhere on the node
    var vk=Object.keys(n0).filter(function(kk){ return /vol|buy|sell|bid|ask|call|put|oi|flow|delta|cvd|qty|size/i.test(kk); });
    console.log('[FEED-PROBE] '+key+' | volume/flow-like NODE keys: '+(vk.length?vk.join(','):'(NONE)'));
    if(last.meta) console.log('[FEED-PROBE] '+key+' | slice.meta: '+JSON.stringify(last.meta));
  }catch(e){ console.log('[FEED-PROBE] error: '+e.message); }
}
function onFeed(sym, feed, j){
  if(sym!=='SPY' && sym!=='QQQ') return;
  if(!j || !j.levels || !j.levels.length) return;
  __feedProbe(sym, feed, j);
...
console.log('[GPTS] v10.31 part1 loaded');
...
// (v10.31-debug) one-time dump of the RAW fiber candle object keys, to see whether
// TradingView candles carry VOLUME (and any buy/sell split) that convertFiberCandles
// currently drops (it copies only o/h/l/c). Remove after the audit.
var __CANDLE_PROBE_DONE = {};
var __PROBE_RESULT = { feed:null, candle:null };   // (v10.31-debug) stashed for DOM read-back
function __candleProbe(sym, raw){
  try{
    if(!raw || !raw.length) return;
    var x=raw[raw.length-1];
    var keys=Object.keys(x);
    var vk=keys.filter(function(kk){ return /vol|buy|sell|bid|ask|qty|size|turnover|amount/i.test(kk); });
    var res={ sym:sym, keys:keys, sample:x, volKeys:vk };
    __PROBE_RESULT.candle = res;
    if(!__CANDLE_PROBE_DONE[sym]){
      __CANDLE_PROBE_DONE[sym]=true;
      console.log('[CANDLE-PROBE] '+sym+' | CANDLE keys: '+keys.join(','));
      console.log('[CANDLE-PROBE] '+sym+' | CANDLE sample: '+JSON.stringify(x));
      console.log('[CANDLE-PROBE] '+sym+' | volume/flow-like CANDLE keys: '+(vk.length?vk.join(','):'(NONE)'));
    }
  }catch(e){ console.log('[CANDLE-PROBE] error: '+e.message); }
}
  ...
  for(var c=0;c<canvases.length;c++){
    ...
    while(node && d<10){
      ...
      if(p && typeof p==='object'){
        try{ if(p.symbol===sym && isCandleArr(p.candles)){ __candleProbe(sym, p.candles); return p.candles; } }catch(e){}
      }
      node=node.return; d++;
    }
  }
...
window.__gptsDebug.PROBE = function(){ return __PROBE_RESULT; };  // (v10.31-debug) raw feed/candle field probe result
...
function feedStatusHtml(){
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:center;color:'+PAL.sub+';font-size:9px;letter-spacing:0.3px">'+
    ...
    '<span id="gpts-probe" title="'+(function(){try{var r=__PROBE_RESULT;var c=r.candle,f=r.feed;var s="";if(c){s+="CANDLE keys: "+c.keys.join(",")+" | volKeys: "+(c.volKeys.length?c.volKeys.join(","):"(NONE)")+" | sample: "+JSON.stringify(c.sample);}if(f){s+=" || NODE keys: "+f.nodeKeys.join(",")+" | volKeys: "+(f.volKeys.length?f.volKeys.join(","):"(NONE)");}return s.replace(/"/g,"&quot;")||"probe: no data yet";}catch(e){return "probe err";}})()+'">feed v10.31</span>'+
    '</div>';
}
