...
console.log('[GPTS] v10.24 part1 loaded');
...
var RUG_POLARITY_VERIFIED = false;   // flip true after confirming sign(pos)==ladder color
...
function kingBlock(){
  ...
  // ---- Header (v10.23 Issue G): TREND BADGE (left) + KING NODE (right) as a
  // matched pair, flex space-between. Trend badge reads the same trendVerdict the
  // (now-continuous, Issue C) SMA feeds, so it is populated from the session open. ----
  var trendBadge = trendBadgeHtml(sym);
  var html='<div style="display:flex;justify-content:space-between;align-items:center;margin:4px 0 3px 0;gap:8px">'+
    trendBadge+
    '<span title="KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s settlement magnet. Below: the roll path, oldest \u2192 newest.">'+kingPxBadge+'</span>'+
  '</div>';
  if(kingK==null){
...
function render(){
  ...
  html+=kingBlock();            // King tracker (header + sparkline + verdict) \u2014 now the TOP
  html+=sep();
