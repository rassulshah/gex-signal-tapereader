...
console.log('[GPTS] v10.8 part1 loaded');
...
function kingSparkline(mv, kingK, px, now, sess){
  var W=236, H=46, padL=4, padR=4, padT=6, padB=6;
  ...
  var d='M '+X(pts[0].t).toFixed(1)+' '+Y(pts[0].k).toFixed(1);
  for(var j=1;j<pts.length;j++){ d+=' H '+X(pts[j].t).toFixed(1)+' V '+Y(pts[j].k).toFixed(1); }
  d+=' H '+X(xMax).toFixed(1);
  var netDir=pts[pts.length-1].k-pts[0].k;
  var lineCol=netDir>0?PAL.longAccent:(netDir<0?PAL.shortAccent:PAL.sub);
  var svg='<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'" style="display:block;width:100%">';
  ...
  svg+='<path d="'+d+'" fill="none" stroke="'+lineCol+'" stroke-width="1.6" stroke-linejoin="round" stroke-linecap="round"/>';
...
function kingBlock(){
  ...
  var spark=kingSparkline(mv, kingK, px, now, sess);
  // Session-drift summary words for the title + tooltip.
  ...
  var sparkTip=('King path today \u2014 strike vs. clock time across the cash session (8:30\u201315:00 CT). '+
    'The staircase steps at each confirmed roll and holds flat in between, so a long flat = pinned and a tight cluster = rapid rolling. '+
    'Net drift '+fmtNum(spark.firstK)+' \u2192 '+fmtNum(spark.lastK)+' ('+driftWord+', '+nRolls+' roll'+(nRolls===1?'':'s')+' today). '+
    'Green line = net higher (bullish migration), red = net lower (bearish); dots mark rolls (green up, red down); gold dot = current King; dashed = current price.').replace(/"/g,'');
