...
console.log('[GPTS] v10.33 part1 loaded');
...
function nodeLifecycleTag(L){
  ...
  var pr=(lc.prob!=null)?(' '+lc.prob+'%'):'';
  return '<span title="'+m.tip+'" style="color:'+m.col+';font-size:8px;font-weight:800;border:1px solid '+m.col+';border-radius:9px;padding:0 4px">'+m.lab+pr+'</span>';
}
...
function nodeMapBlock(){
  ...
  function row(L){
    ...
    return '<div style="display:flex;align-items:center;gap:5px;font-size:10px;padding:2px 5px;border-radius:5px;background:'+bg+'">'+
      ...
      (lifeHtml?'<span style="width:auto;text-align:left">'+lifeHtml+'</span>':'')+
      stageCell+
      boCell+
