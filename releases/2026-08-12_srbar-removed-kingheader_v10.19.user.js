...
console.log('[GPTS] v10.19 part1 loaded');
...
function kingBlock(){
  ...
  if(kingK!=null){
    ...
    if(off!=null && off!==0){
      ...
      var arr = above?'\u2191':'\u2193';
      arrHtml='<span title="King is '+Math.abs(off)+' strike'+(Math.abs(off)===1?'':'s')+' '+(above?'ABOVE price (magnet pulling up)':'BELOW price (magnet pulling down)')+'." style="color:'+arrC+';font-weight:800;font-size:15px;line-height:1">'+arr+'</span>';
    }
    kingPxBadge=
