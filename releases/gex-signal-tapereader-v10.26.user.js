...
console.log('[GPTS] v10.26 part1 loaded');
...
function gatekeeper(sym){
  ...
  // GATEKEEPER = the NEAREST such node (the first blocker price meets).
  between.sort(function(a,b){ return a.dist-b.dist; });
  var gk=between[0];
...
function gatekeeperBlock(){
  ...
  var hdr='<div style="display:flex;align-items:center;gap:6px;margin:0 2px 5px">'+gate+stepIcon(4)+
...
function nodeRoleBadge(L){
  ...
  if(L.isKing){
    ...
    label=(L.rugType==='Reverse-Rug')?'RevRug-floor':'Rug-ceiling'; col='#b58bff';
    tip='Rug pattern ceiling ('+(L.rugType||'Rug')+') \u2014 positive-gamma cap over a negative-gamma floor.';
  } else if(L.isRugFloor){
    label=(L.rugType==='Reverse-Rug')?'RevRug-ceiling':'Rug-floor'; col='#b58bff';
    tip='Rug pattern floor ('+(L.rugType||'Rug')+') \u2014 negative-gamma target below the positive-gamma cap.';
  } else if(L.isStack){
    label='Double-Stack'; col=PAL.longAccent;
    tip='Double-Stack \u2014 adjacent stacked nodes forming a strong-BOUNCE shelf; price tends to bounce cleanly off it.';
  } else if(L.isCluster){
    label='Cluster'; col=PAL.blue;
    tip='Cluster \u2014 several large nodes grouped in a tight band; price tends to PIN / CHOP here (poor R:R \u2014 avoid the middle).';
  } else if(L.role==='Floor'){
    label='Floor'; col=PAL.longAccent; tip='Floor \u2014 support node below price (dealers buy as price declines into it).';
  } else if(L.role==='Ceiling'){
    label='Ceiling'; col=PAL.shortAccent; tip='Ceiling \u2014 resistance node above price (dealers sell as price rises into it).';
  }
  if(!label) return '';
...
function nodeMapBlock(){
  ...
  var html=sectionHdrRight('Node Map', (m.ok?rgChip:''), hdrTip);
  if(!m.ok){ html+='<div style="color:'+PAL.sub+';padding:2px 6px;font-size:11px">Node Map \u2014 waiting on node data\u2026</div>'; return html; }
  // regime instruction line (Issue L: regime-as-instruction)
  if(rg.label && rg.label!=='Forming'){
    var instr = /Trend/.test(rg.label) ? ('Enter on pullbacks'+(rg.target!=null?(' toward '+fmtNum(rg.target)):'')+'.')
              : rg.label==='Whipsaw' ? 'Fade the edges, avoid the middle.'
              : rg.label==='Rainbow Road' ? 'Stand aside \u2014 no clean edge.'
              : 'Low conviction \u2014 wait for structure.';
    html+='<div style="font-size:9.5px;color:'+rgCol+';font-weight:700;margin:0 2px 4px">'+instr+'</div>';
  }
  ...
  html+='<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;margin:1px 2px 4px">'+
    strongChip(m.strongSup,'Sup',PAL.longAccent)+strongChip(m.strongRes,'Res',PAL.shortAccent);
  if(m.againstKing && m.kingK!=null){
    html+='<span title="Price is moving AWAY from the King \u2014 the magnet behind may pull it back (snapback risk)." style="color:'+PAL.amber+';font-size:9px;font-weight:700">\u21a9 King '+fmtNum(m.kingK)+' behind</span>';
  }
  html+='</div>';
  ...
  var rug=m.rug;
  if(rug && rug.ok){
    var isRug=(rug.type==='Rug');
    var rugCol = rug.shown ? (isRug?PAL.shortAccent:PAL.longAccent) : PAL.sub;
    var conf = (rug.confirm && (rug.confirm.downGrowing||rug.confirm.lowUpside)) ? '' : ' (unconfirmed \u2014 flow not yet aligned)';
    var head = rug.shown ? (isRug?'\u26a0 RUG forming':'\u26a0 REVERSE-RUG forming')
                         : (isRug?'RUG candidate':'REVERSE-RUG candidate')+' \u2014 verify polarity';
    html+='<div title="'+(rug.why+' Targets: '+(rug.targets||[]).map(fmtNum).join(', ')+'.').replace(/"/g,'')+'" style="font-size:9.5px;color:'+rugCol+';font-weight:800;margin:0 2px 4px">'+head+' \u00b7 '+fmtNum(rug.ceilK)+' over '+fmtNum(rug.floorK)+conf+'</div>';
  }
  ...
  // (v10.26-prep) DOUBLE-STACK callout: adjacent stacked nodes -> strong-bounce shelf.
  var stk=m.stack;
  if(stk && stk.ok){
    stk.stacks.forEach(function(s){
      var span=(s.lo===s.hi)?fmtNum(s.lo):(fmtNum(s.lo)+'\u2013'+fmtNum(s.hi));
      html+='<div title="Double-Stack: '+s.n+' adjacent significant nodes within '+STACK_GAP+' strike \u2014 a strong-bounce shelf; price tends to bounce cleanly off it (fade into it)." style="font-size:9.5px;color:'+PAL.longAccent+';font-weight:800;margin:0 2px 4px">\u2b1b\u2b1b Double-Stack '+span+' \u2014 strong-bounce shelf</div>';
    });
  }
  // (v10.26-prep) CLUSTER callout: grouped nodes -> pins/chops region (avoid the middle).
  var cl=m.cluster;
  if(cl && cl.ok){
    cl.regions.forEach(function(r){
      html+='<div title="Cluster: '+r.n+' large nodes grouped within '+CLUSTER_BAND+' strikes \u2014 price tends to PIN / CHOP through this region. Poor R:R in the middle; trade the edges." style="font-size:9.5px;color:'+PAL.blue+';font-weight:800;margin:0 2px 4px">\u25a6 Cluster '+fmtNum(r.lo)+'\u2013'+fmtNum(r.hi)+' ('+r.n+' nodes) \u2014 pin/chop zone</div>';
    });
  }
...
function kingSparkline(mv, kingK, px, now, sess, verdictCol){
  ...
  if(pxRef!=null){
    ...
    var yp = Y(clampedPx);
    priceLineY = yp;
    var lineOp = pxStale ? 0.28 : 0.6;
    svg+='<line x1="'+padL+'" y1="'+yp.toFixed(1)+'" x2="'+(W-padR)+'" y2="'+yp.toFixed(1)+'" stroke="'+PAL.blue+'" stroke-width="1" stroke-dasharray="2 3" opacity="'+lineOp+'"/>';
  ...
  // (v10.23 Issue A) LABEL the King strike on the gold dot (gold, above-left of the dot).
  var curK = pts[pts.length-1].k;
  var kLx = Math.max(padL+2, parseFloat(lx)-6);
  var kLy = Math.max(padT+8, parseFloat(ly)-6);
  svg+='<text x="'+kLx.toFixed(1)+'" y="'+kLy.toFixed(1)+'" text-anchor="end" fill="'+PAL.gold+'" style="font-size:9px;font-weight:800;font-variant-numeric:tabular-nums">\uD83D\uDC51'+fmtNum(curK)+'</text>';
  // (v10.23 Issue A) LABEL the price line at its RIGHT end with value + signed offset,
  // e.g. '771.9 (-1.1)'. Sign = price vs King (\u2212 below King, + above).
  if(pxRef!=null && priceLineY!=null){
    var offv = (kingK!=null) ? (pxRef-kingK) : null;   // price relative to King
    var offTxt = (offv!=null) ? (' ('+(offv>=0?'+':'\u2212')+Math.abs(offv).toFixed(1)+')') : '';
    var labY = priceLineY - 3;
    if(labY < padT+8) labY = priceLineY + 10;    // keep it on-canvas if line is near the top
    svg+='<text x="'+(W-padR-2)+'" y="'+labY.toFixed(1)+'" text-anchor="end" fill="'+PAL.blue+'" style="font-size:9px;font-weight:700;font-variant-numeric:tabular-nums">'+fmtNum(pxRef)+offTxt+'</text>';
  }
  svg+='</svg>';
...
function kingBlock(){
  ...
  if(kingK!=null){
    ...
    if(off!=null && off!==0){
      ...
      arrHtml='<span title="King is '+Math.abs(off)+' strike'+(Math.abs(off)===1?'':'s')+' '+(above?'ABOVE price':'BELOW price')+' (offset). + = above, \u2212 = below." style="color:'+arrC+';font-weight:800;font-size:14px;line-height:1;font-variant-numeric:tabular-nums">'+sign+Math.abs(off)+arr+'</span>';
    }
    kingPxBadge=
      '<span title="King strike '+fmtNum(kingK)+' vs SPY '+(px!=null?fmtNum(px):'\u2013')+'. Crown+top = King (settlement magnet), below = current price. Arrow: \u2191 King above price, \u2193 below; none when equal." '+
        'style="display:inline-flex;align-items:center;gap:7px;padding:3px 12px;border:1.5px solid '+PAL.gold+';border-radius:16px;background:'+PAL.card+'">'+
        '<span style="font-size:14px;line-height:1">\uD83D\uDC51</span>'+
        '<span style="display:inline-flex;flex-direction:column;align-items:center;line-height:1.05">'+
          '<span style="color:'+PAL.gold+';font-weight:800;font-size:15px;font-variant-numeric:tabular-nums">'+fmtNum(kingK)+'</span>'+
          '<span style="width:100%;height:1px;background:'+PAL.line+';margin:1px 0"></span>'+
  ...
  function sideMagnet(node, isSup){
    ...
    if(!node){ return '<span style="display:inline-flex;flex-direction:column;align-items:center;justify-content:center;padding:4px 10px;height:42px;box-sizing:border-box;border-radius:16px;background:'+PAL.card+';border:1.5px solid '+PAL.line+';opacity:.6"><span style="font-size:8px;font-weight:800;color:'+PAL.sub+';letter-spacing:.4px">'+lbl+'</span><span style="width:100%;height:1px;background:'+PAL.line+';margin:2px 0"></span><span style="color:'+PAL.sub+';font-size:11px">\u2013</span></span>'; }
    ...
    return '<span title="'+tip.replace(/"/g,'')+'" style="display:inline-flex;flex-direction:column;align-items:center;justify-content:center;padding:4px 10px;height:42px;box-sizing:border-box;border-radius:16px;background:'+PAL.card+';border:1.5px solid '+col+'">'+
      '<span style="font-size:8px;font-weight:800;color:'+col+';letter-spacing:.4px">'+lbl+'</span>'+
      '<span style="width:100%;height:1px;background:'+PAL.line+';margin:2px 0"></span>'+
      '<span><span style="font-weight:800;font-size:14px;color:'+col+';font-variant-numeric:tabular-nums">'+fmtNum(node.k)+'</span> <span style="color:'+PAL.sub+';font-size:9px;font-weight:600;font-variant-numeric:tabular-nums">'+node.pct+'%</span></span>'+
    '</span>';
  }
  var supBadge = sideMagnet(nm?nm.strongSup:null, true);
  var resBadge = sideMagnet(nm?nm.strongRes:null, false);
  var html='<div style="display:flex;justify-content:center;align-items:center;gap:7px;margin:4px 0 3px 0">'+
    supBadge+
    '<span title="KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s EOD settlement magnet.">'+kingPxBadge+'</span>'+
    resBadge+
  '</div>'+
...
function accumBlock(){
  ...
  // (v10.23 Issue D) NAME THE MECHANISM. Compact headline pill = direction; the
  // full 'resistance X building, support Y fading' sentence rides just under it.
  var forming = !haveSrb || srb.forming;
  var headline = forming ? (haveSrb?'Forming':'Balanced')
    : (srb.dom==='support'?'Bullish imbalance':(srb.dom==='resistance'?'Bearish imbalance':'Balanced'));
  function fmtK(k){ return k!=null ? fmtNum(k) : '\u2013'; }
  // mechanism sentence: <gaining side><strike> building, <fading side><strike> fading
  var mechTxt='';
  if(!forming && srb.dom==='resistance'){
    mechTxt='resistance '+fmtK(srb.gain)+' building, support '+fmtK(srb.fade)+' fading';
  } else if(!forming && srb.dom==='support'){
    mechTxt='support '+fmtK(srb.gain)+' building, resistance '+fmtK(srb.fade)+' fading';
  } else if(haveSrb && srb.forming){
    mechTxt='not enough rate data yet';
  } else {
    mechTxt='neither side clearly gaining';
  }
  ...
  var boardTip = (haveSrb
    ? ('S/R IMBALANCE \u2014 the DIVERGENCE in build-RATE between the two sides near price (not who is bigger). '
       +(forming?'Forming: not enough rate samples yet.':(srb.dom==='resistance'?('Bearish: '+mechTxt+' \u2014 the ceiling is strengthening while the floor gives way, so a pullback tends to fail and roll down.'):('Bullish: '+mechTxt+' \u2014 the floor is strengthening while overhead gives way, so dips tend to hold.')))
       +' Committed on 3m bar close (de-flickered). Nearest floor '+nfTxt+', nearest ceiling '+ncTxt+'.')
    : 'S/R IMBALANCE \u2014 not enough node data yet.').replace(/"/g,'');
  ...
  var html=sectionHdrRight('S/R Imbalance', srBiasHdrRight, 'S/R IMBALANCE: measures the DIVERGENCE in rate-of-change between support and resistance near price \u2014 one side BUILDING while the other FADES is what moves price, not which side is statically bigger. Bearish = resistance building while support fades (pullback fails, rolls down); bullish = the mirror. Committed on 3m bar close so it does not flicker. The sentence names the exact strikes gaining/fading.');
  if(mechTxt){
    html+='<div style="font-size:10.5px;line-height:1.4;color:'+PAL.ink+';margin:1px 2px 3px;"><span style="font-weight:800;color:'+netCol+'">'+(forming?'':(srb.dom==='resistance'?'Bearish imbalance':srb.dom==='support'?'Bullish imbalance':''))+'</span>'+(forming?'':' \u2014 ')+mechTxt+'</div>';
  }
  ...
  // (v10.19) S/R Bias BALANCE BAR removed per spec. The srBattle read now lives in
  // the READ synthesis at top ("support/resistance winning") and the tradeable
  // crossover still shows as the banner above. Node ladder + PROJ below remain —
  // those ARE the S/R nodes the READ refers to.
  ...
  // (v10.23 Issue H/D) COLOR BY DIRECTIONAL MEANING, not the raw action.
  // A building RESISTANCE (ceiling strengthening) is BEARISH -> red; a building
  // SUPPORT (floor strengthening) is BULLISH -> green; fading flips each; steady
  // is neutral grey. Prior code colored a building ceiling gold/green, which read
  // as non-bearish — backwards. Rule locked with user 2026-08-12 (mockup review).
  function stateColor(label, side){
    var isRes = (side==='above');
    if(label==='Building') return isRes ? PAL.shortAccent : PAL.longAccent;  // res build=red, sup build=green
    if(label==='Fading')   return isRes ? PAL.longAccent  : PAL.shortAccent; // res fade=green, sup fade=red
    return PAL.sub;   // Steady = neutral grey
  }
...
function render(){
  ...
  html+=sep();
  html+=symSignalsHdr('SPY');   // (v10.20) BO / SPY Signals moved ABOVE S/R Bias
  html+=signalGrid();
  html+=sep();
  html+=accumBlock();           // S/R Bias node ladder + PROJ (now below BO)
