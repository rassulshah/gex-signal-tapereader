...
// --- Node history strip (1-minute %King samples read from the tape) ---
var HIST_SAMPLE_MS = 60000;  // one sample per minute
var HIST_MAX = 8;            // keep/show the last 8 points (~8 min)
var HIST_STEADY_BAND = 6;    // <= this net %-pt move over the window = Steady
var HIST_RAPID_STEP = 10;    // >= this jump in the last minute = Rapid
var HIST_SHARP_STEP = 8;     // a minute-over-minute %King change >= this is a SHARP move, colored
var GK_MAX_DIST = 5;
var STATS_DAYS = 5;
var READ_NEARX = 0.25;
...
console.log('[GPTS] v9.2 part1 loaded');
...
function tapeCells(table){
  // Walk the subtree; when an element's leading token is a value, record that
  // token and do NOT recurse into it (its own text leads, children are chips).
  var out=[];
  (function walk(node){
...
function readTapeFromDOM(sym){
  ...
  // after a strike as that strike's nearest-expiry %King (sign kept, because
  // Skylit's nearest %King can legitimately be negative). The $K King dollar
  // cell marks the King row. ----
  pct={}; kingK=null; count=0;
  var cells=tapeCells(table);
  ...
  for(var L=0;L<cells.length;L++){
    ...
    if(sm){
      // A bare number that looks like a strike opens a new row. Guard the
      // range so stray ladder numbers/dates don't masquerade as strikes.
    ...
    // King dollar cell -> this strike is King. Lock the row so a trailing
    // change chip can't overwrite the King's 100.
    ...
    // First %King for this strike = nearest-expiry strength (sign kept).
    if(!haveVal){
...
// Returns null if the leaf isn't a standalone percent value.
...
function sampleTapeHistory(sym){
  ...
  for(var key in tp.pct){
    ...
    // Sample at most once per minute; if within the same minute, update the
    // latest point in place so it tracks the live tape between samples.
  ...
  // Track the King strike over time (1-min samples) to detect ROLLING.
...
function nodeHistory(sym, k){
  ...
  return rec.seq.map(function(p){ return p.v; });
}
...
function histTrend(seq){
  ...
  // RAPID signal (separate from the slow badge): the move over the LAST 2
  // minutes. +HIST_RAPID_STEP or more = Rapid Accumulation (surging in);
  // -HIST_RAPID_STEP or less = Rapid Unwinding (falling off fast). 0 = neutral.
  var recentMove = (n>=3) ? (seq[n-1]-seq[n-3]) : (seq[n-1]-seq[n-2]);
  var rapidDir = 0;
...
// DATA layer export: return the full recorder DB (node snapshots + outcome
// events, keyed by day). Also console-logs a compact summary. Copy the return
// value (or JSON.stringify it) to feed the analytics / LLM prediction layer.
...
// Convenience: recorder DB as a JSON string ready to copy out.
window.__gptsDebug.dumpRecorderJSON = function(){ return JSON.stringify(recorderLoad()); };
// Wipe the recorder store only (does NOT touch signals/state).
...
function combinedGrid(rows){
  ...
  // Trimmed to the essentials: Strike, %King, BO, FT, Void. (PB, Conf, Go
  // removed to save vertical space.)
  var rowDefs=[
    {key:'STRIKE', lbl:'Strike'},
    {key:'PCT',    lbl:'%King'},
    {key:'BO',     lbl:'BO'},
    ...
    setups.forEach(function(s, ci){
      ...
      if(rd.key==='STRIKE'){
        ...
        var cellTxt='<span style="color:'+accent+';font-weight:800">'+fmtNum(s.strike)+'</span>'+
          '<span style="color:'+accent+';font-size:8px;font-weight:800;margin-left:3px;border:1px solid '+accent+';border-radius:3px;padding:0 2px">'+tag+'</span>';
...
function targetLadder(){
  ...
  // Prefer the TAPE's King as T2 so targets stay consistent with the tape.
  var tk=tapeKingStrike('SPY');
  if(tk!=null && (longSide ? tk>px : tk<px)) t2=tk;
...
function targetMeta(){
  ...
  return {
    ...
    t2: lad.t2,
    t1star: lad.t1star,
    t1Label: lad.t1!=null ? ('T1 '+fmtNum(lad.t1)+' Gk') : null,
    t2Label: lad.t2!=null ? ('T2 '+fmtNum(lad.t2)+' K') : null
  };
}
...
function futureStructureSummary(sym){
  ...
  for(var i=0;i<walls.length;i++){
    ...
    var htrend = (hist && hist.length>=2) ? histTrend(hist) : null;
...
function structuralReadHtml(){
  ...
  var hdrRight = trendBadgeHtml()+
      (meta.t1Label?'<span title="First meaningful target in the active direction." style="color:'+PAL.blue+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+PAL.blue+';border-radius:20px">'+meta.t1Label+'</span>':'')+
      (meta.t2Label?'<span title="Next meaningful target beyond T1 in the same direction." style="color:'+PAL.gold+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+PAL.gold+';border-radius:20px">'+meta.t2Label+'</span>':'');
  var html = sectionHdrRight('Read', hdrRight, 'READ: the coordinated one-line take on structure right now \u2014 what price is doing relative to the nearest node, the trend (Up/Dn/Side/NA), and the two targets: T1 (Gatekeeper, the first checkpoint) and T2 (King, the settlement magnet). The PROJECTED row flags where support/resistance is forming from the strongest accumulating node.');
...
function kingBlock(){
  ...
  var hdrRight = '<span title="'+(roll>0?'King is rolling to higher strikes \u2014 bullish dealer intent, up-thesis playing out.':(roll<0?'King is rolling to lower strikes \u2014 bearish dealer intent, down-thesis playing out.':'King strike is stable \u2014 dealers holding the settlement target.'))+'" style="color:'+rollC+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+rollC+';border-radius:20px">'+rollTxt+'</span>';
  var html=sectionHdrRight('King', hdrRight, 'KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s center of gravity and settlement magnet. This section tracks it through the session: its current strike, distance from price, and whether it is rolling UP (bullish dealer intent) or DOWN (bearish). A moving King is a directional signal; a stable King anchors the range.');
  if(kingK==null){
    html+='<div style="color:'+PAL.sub+';padding:2px 6px;font-size:11px">Waiting on tape\u2026</div>';
    return html;
  }
  var distTxt = (px!=null) ? (Math.abs(kingK-px).toFixed(0)+' strike'+(Math.abs(kingK-px)>=2?'s':'')+' '+(kingK>px?'above':(kingK<px?'below':'at'))+' price') : '';
  var kingTip = ('King Node '+fmtNum(kingK)+': the strike with the largest absolute dealer exposure on the board \u2014 where Market Makers hold the most exposure and price tends to gravitate near expiry. '+(distTxt?('Currently '+distTxt+'. '):'')+(seq.length>=2?('Session path: '+detail+'.'):'Tracking its movement through the session.')+(moved!==0?(' Net move '+(moved>0?'+':'')+moved+' strikes '+(moved>0?'up (bullish)':'down (bearish)')+'.'):' Holding at the same strike.')).replace(/"/g,'');
  html+='<div title="'+kingTip+'" style="display:flex;justify-content:space-between;align-items:center;padding:5px 9px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;margin:2px 0">'+
    '<span style="color:'+PAL.gold+';font-weight:800;font-size:13px">K '+fmtNum(kingK)+'</span>'+
    '<span style="color:'+PAL.sub+';font-size:10px">'+(distTxt||'')+'</span>'+
    '<span style="color:'+rollC+';font-size:11px;font-weight:800">'+(roll>0?'\u2197':(roll<0?'\u2198':'\u2192'))+(moved!==0?(' '+(moved>0?'+':'')+moved):'')+'</span>'+
  '</div>';
  return html;
}
...
function accumBlock(){
  ...
  var px = STATE.SPY.price;
  // Accumulation color is SIDE-AWARE: a node building as SUPPORT (below price)
  // is green; a node building as RESISTANCE (above price) is yellow, so the
  // trader can tell "floor forming" from "ceiling forming" at a glance.
  // Dissipating stays red, Steady stays blue.
  ...
  // SIDE-AWARE (see stColor above): Building support = green, Building
  // resistance = yellow, Fading = red, Steady = blue.
  ...
  function rowHtml(r){
    ...
    var tip = (roleTip(r.role)+' Strike '+fmtNum(r.k)+' \u2014 '+r.pct+'% of King, '+distTxt+'. Flow: '+slow+rapidTxt+' ('+rateTxt+', net '+netTxt+').'+rollTxt+' Timestamped %King: '+(tsDetail||(seq.length?seq.join(' -> '):'n/a'))+'. '+read).replace(/"/g,'');
    ...
    return '<div title="'+tip+'" style="display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:7px;align-items:stretch;padding:3px 7px;margin:2px 0;min-height:36px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-left:3px solid '+left+';border-radius:7px">'+
      ...
      '<span style="display:flex;flex-direction:column;justify-content:center;gap:2px;min-width:0">'+
        '<span title="'+roleTip(r.role).replace(/"/g,'')+'" style="color:'+PAL.ink+';font-size:10px;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+label+' · '+r.pct+'%'+rollHtml(r)+'</span>'+
      ...
      '<span style="display:flex;align-items:center;justify-content:flex-end">'+
        '<span title="'+stateTip+'" style="color:'+col+';font-size:9.5px;font-weight:700;padding:1px 7px;border:1px solid '+col+';border-radius:20px;white-space:nowrap">'+slow+rapidGlyph(r)+'</span>'+
      '</span>'+
    '</div>';
  }
