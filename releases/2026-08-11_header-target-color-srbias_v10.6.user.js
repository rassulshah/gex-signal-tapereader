...
console.log('[GPTS] v10.6 part1 loaded');
...
function structuralReadHtml(){
  ...
  var trendCounter = tv.win ? (' ('+tv.up+'/'+tv.win+' bars)') : '';
...
function kingBlock(){
  ...
  // ---- Header ----
  // #6 (v10.6) Removed the per-day roll-count badge from the header (knowing it
  // is "42" adds no signal). Kept: current King, distance, net session drift.
  var hdrRight =
  ...
  if(mv.length){
    ...
    // #5 (v10.6) Subtle \u2192 connector between nodes. Faint (PAL.sub) and tiny
    // (8px), with ZERO horizontal padding + a slight negative margin so it sits
    // inside the existing gap and does NOT push the chips further apart.
    var arrowJoin='<span aria-hidden="true" style="flex:0 0 auto;color:'+PAL.sub+';opacity:0.55;font-size:8px;padding:0;margin:0 1px;line-height:1">\u2192</span>';
...
function accumBlock(){
  ...
  // LOCAL = relativeRead() comparing the nearest building support vs resistance.
  ...
  var html=sectionHdrRight('S/R Bias', srBiasHdrRight, 'S/R BIAS: support vs resistance. The nodes list shows only the strikes that matter right now (Accumulating / Dissipating, plus the King); PROJ flags the strongest forming floor & ceiling. The header badge + balance bar give the BOARD tilt (day bias); the LOCAL tile is the nearest building support vs resistance (immediate edge). Green = building support (below), yellow = building resistance (above), red = dissipating.');
  ...
  if((net && net.bias!=='balanced') || (rel && (rel.sup || rel.res))){
    ...
    // ---- Render: balance bar + BOARD / LOCAL tiles (header already emitted) ----
