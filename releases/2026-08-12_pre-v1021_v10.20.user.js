...
console.log('[GPTS] v10.20 part1 loaded');
...
function kingBlock(){
  ...
  if(kingK!=null){
    ...
    if(off!=null && off!==0){
      ...
      var arr = above?'\u2191':'\u2193';
      arrHtml='<span title="King is '+Math.abs(off)+' strike'+(Math.abs(off)===1?'':'s')+' '+(above?'ABOVE price (magnet pulling up)':'BELOW price (magnet pulling down)')+'." style="color:'+arrC+';font-weight:800;font-size:14px;line-height:1;font-variant-numeric:tabular-nums">'+arr+Math.abs(off)+'</span>';
    }
    kingPxBadge=
  ...
  // ---- Header: centered badge, no title text ----
  var html='<div title="KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s settlement magnet. Below: the roll path, oldest \u2192 newest." style="display:flex;justify-content:center;align-items:center;margin:4px 0 3px 0">'+kingPxBadge+'</div>';
  if(kingK==null){
