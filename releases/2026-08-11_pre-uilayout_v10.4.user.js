...
console.log('[GPTS] v10.4 part1 loaded');
...
function kingBlock(){
  ...
  var html=sectionHdrRight('\uD83D\uDC51 King', hdrRight, 'KING: the strike with the largest absolute dealer exposure \u2014 the day\u2019s center of gravity and settlement magnet. Header: current King, distance from price, net session drift, and today\u2019s roll count. Below: a one-line read, quick stats (time pinned / session drift / last-hour rolls), and the roll path (most-recent first, older scroll right).');
  if(kingK==null){
    html+='<div style="color:'+PAL.sub+';padding:2px 6px;font-size:11px">Waiting on tape\u2026</div>';
    return html;
  }

  // ---- Derived stats from the roll timeline ----
  // pinned time: how long since the last confirmed roll (or since first seen).
  ...
  // rolls in the last 60 min (exclude the seed move dir:0).
  var hourAgo=now-3600000, lastHrRolls=0;
  for(var i=0;i<mv.length;i++){ if(mv[i].dir!==0 && mv[i].t>=hourAgo) lastHrRolls++; }
  var sessTxt = mv.length? (fmtNum(firstK)+'\u2192'+fmtNum(kingK)) : (fmtNum(kingK)+' (flat)');

  // ---- KING READ (descriptive + structural implication, no trade advice) ----
  var driftWord = net>0?'up':(net<0?'down':'sideways');
  var driftClause = (net!==0)
    ? ('Drifted '+driftWord+' '+fmtNum(firstK)+'\u2192'+fmtNum(kingK)+' ('+(net>0?'bullish':'bearish')+' migration)')
    : ('Held near '+fmtNum(kingK)+' all session');
  var pinClause = (pinnedM!=null && pinnedM>=8)
    ? (', now pinned '+pinnedTxt)
    : (lastHrRolls>=2 ? ', actively rolling' : '');
  var locClause = (px!=null)
    ? (Math.round(Math.abs(kingK-px))===0 ? ' at price' : (kingK>px?' above price':' below price'))
    : '';
  var atPriceNow = (px!=null) && (Math.round(Math.abs(kingK-px))===0);
  var stablePinned = (pinnedM!=null && pinnedM>=15);   // held current strike >=15m
  var implication;
  if(stablePinned && atPriceNow){ implication=' Settlement gravity right here and holding \u2014 range-pin behavior; a confirmed roll away is what would start a trend leg.'; }
  else if(stablePinned){ implication=' Settled and holding for now \u2014 the '+(net<0?'lower':'higher')+' migration has paused; watch for either a fresh roll or price converging into it.'; }
  else if(lastHrRolls>=2){ implication=' Unstable \u2014 settlement target still moving; treat levels as provisional until it settles.'; }
  else if(atPriceNow){ implication=' Settlement gravity right here \u2014 a hold keeps it a range pin; a confirmed roll away starts a trend leg.'; }
  else if(net<0){ implication=' Dealers settling lower \u2014 rallies into the King are the spots to watch for rejection.'; }
  else if(net>0){ implication=' Dealers settling higher \u2014 dips toward the King are where support tends to appear.'; }
  else { implication=' Range-anchored \u2014 no directional migration yet.'; }
  var readTxt = driftClause+pinClause+locClause+'.'+implication;
  html+='<div style="background:rgba(227,195,65,0.06);border:1px solid rgba(227,195,65,0.30);border-radius:8px;padding:5px 9px;margin:3px 0;font-size:11px;line-height:1.45">'+
    '<span style="color:'+PAL.gold+';font-weight:800;font-size:9.5px;letter-spacing:.3px">KING READ \u00b7 </span>'+
    '<span style="color:'+PAL.ink+'">'+readTxt+'</span>'+
  '</div>';

  // ---- Quick-stat tiles ----
  function tile(lbl,val,vc){ return '<div style="flex:1;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:7px;padding:3px 6px;text-align:center;min-width:0">'+
    '<div style="color:'+PAL.sub+';font-size:8px;font-weight:700;letter-spacing:.3px">'+lbl+'</div>'+
    '<div style="color:'+(vc||PAL.ink)+';font-size:12px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+val+'</div></div>'; }
  html+='<div style="display:flex;gap:5px;margin:3px 0">'+
    tile('PINNED', pinnedTxt, PAL.ink)+
    tile('SESSION', sessTxt, netC)+
    tile('LAST HR', lastHrRolls+(lastHrRolls===1?' roll':' rolls'), lastHrRolls>=2?PAL.amber:PAL.sub)+
  '</div>';

  // ---- Roll path: MOST-RECENT FIRST (left), older scroll right, dedup consecutive ----
  // Build display list newest->oldest, collapsing consecutive same-strike entries.
  var pathHtml='';
  if(mv.length){
    var disp=[];
    for(var j=mv.length-1;j>=0;j--){
      var m=mv[j];
      if(disp.length && Math.abs(disp[disp.length-1].k-m.k)<0.001){ continue; } // collapse consecutive dupes
      disp.push(m);
    }
    var chips=[];
    for(var c=0;c<disp.length;c++){
      ...
      var gl = isNow?'' : (mm.dir>0?'\u2191':(mm.dir<0?'\u2193':''));
      var stamp = isNow ? ('now'+(pinnedM!=null?('\u00b7'+pinnedTxt):'')) : fmtClock(mm.t);
      var bg = isNow?'background:rgba(227,195,65,0.10);':'';
      chips.push('<span style="color:'+cc+';font-weight:'+(isNow?'800':'700')+';border:1px solid '+cc+';border-radius:4px;padding:1px 5px;'+bg+'">'+fmtNum(mm.k)+gl+'<span style="color:'+PAL.sub+';font-size:8px"> '+stamp+'</span></span>');
    }
    var arrowJoin='<span style="color:'+PAL.line+';font-size:9px;padding:0 1px">\u2190</span>';
  ...
  var pathTip=('Today\u2019s King roll path, most-recent first (left) \u2014 older rolls scroll right. Consecutive repeats are collapsed. Green \u2191 = rolled higher (bullish), red \u2193 = lower (bearish); gold = current. '+moveCount+' confirmed roll'+(moveCount===1?'':'s')+' today.').replace(/"/g,'');
  html+='<div style="padding:5px 8px;background:'+PAL.card+';border:1px solid '+PAL.line+';border-radius:8px;margin:2px 0">'+
    '<div style="color:'+PAL.sub+';font-size:8px;font-weight:700;letter-spacing:.4px;margin-bottom:4px">KING PATH \u00b7 recent first \u00b7 scroll \u2192</div>'+
    '<div title="'+pathTip+'" style="display:flex;gap:0;align-items:center;overflow-x:auto;white-space:nowrap;padding-bottom:2px;font-size:10.5px;font-variant-numeric:tabular-nums">'+pathHtml+'</div>'+
  '</div>';
  return html;
}
