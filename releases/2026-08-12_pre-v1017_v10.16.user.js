...
console.log('[GPTS] v10.16 part1 loaded');
...
function feedStatusHtml(){
  ...
  return '<div style="display:flex;justify-content:space-between;align-items:center;color:'+PAL.sub+';font-size:9px;letter-spacing:0.3px">'+
    ...
    '<span style="display:flex;align-items:center;gap:8px">'+
      '<span title="Download today\u2019s labeled data (snapshots + outcomes) as gex_'+ (TODAY||'DATE') +'.json. Drop it into your AI Drive /GEX-Signal-Tapereader/daily-data/ folder for the end-of-day review." onclick="window.__gptsDebug&&window.__gptsDebug.saveDayToFile&&window.__gptsDebug.saveDayToFile()" style="cursor:pointer;color:'+PAL.blue+';border:1px solid '+PAL.line+';border-radius:10px;padding:0 6px">\uD83D\uDCE5 Save Day</span>'+
      '<span>feed v10.16</span>'+
    '</span>'+
    '</div>';
}
...
function _await(txt){ return '<div style="margin:0 0 14px 30px;font-size:10px;color:'+PAL.sub+';background:#0d1420;border-left:2px solid '+PAL.blue+';padding:6px 8px;border-radius:0 6px 6px 0">'+txt+'</div>'; }
...
function analysisBlock(){
  ...
  h+='<div style="font-size:10px;color:'+PAL.sub+';margin:0 0 6px 30px">King \u00b7 price \u00b7 S/R dominance \u2014 scroll \u2192</div>';
  h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px"><div style="overflow-x:auto;padding-bottom:4px">'+timelineSvg(sym,st)+'</div></div>';

  // ---- STEP 2: King vs price convergence ----
  h+=_step(2,'Was the King pulling price?');
  h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px"><div style="overflow-x:auto">'+convergenceSvg(sym,st)+'</div></div>';

  // ---- STEP 3: node lifecycle (scrollable) ----
  h+=_step(3,'What the nodes did (multi-strike)');
  h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px;max-height:150px;overflow-y:auto">';
  ...
  h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px">';
  ...
  h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px 10px">';
  ...
  if(R&&(R.worked||R.missed)){
    h+='<div style="margin:0 0 8px 30px;display:flex;gap:8px">'+
    ...
    if(R.why) h+='<div style="margin:0 0 14px 30px;background:#12161f;border:1px solid '+PAL.line+';border-radius:10px;padding:8px 10px"><div style="font-size:10px;font-weight:800;color:'+PAL.blue+';margin-bottom:3px">\uD83D\uDD0D WHY</div><div style="font-size:10px;color:#c8d3df;line-height:1.5">'+R.why+'</div></div>';
  } else {
  ...
  if(R&&(R.discovered||( R.recs&&R.recs.length))){
    if(R.discovered) h+='<div style="margin:0 0 8px 30px;background:#160f1f;border:1px solid #2e2140;border-radius:10px;padding:8px 10px"><div style="font-size:10px;font-weight:800;color:#b58ce0;margin-bottom:3px">\uD83D\uDCA1 DISCOVERED</div><div style="font-size:10px;color:#c8d3df;line-height:1.5">'+R.discovered+'</div></div>';
    (R.recs||[]).forEach(function(rc,i){ var col=[PAL.shortAccent,PAL.amber,PAL.blue][i]||PAL.sub;
      h+='<div style="margin:0 0 6px 30px;display:flex;gap:8px;align-items:flex-start;background:#12161f;border:1px solid '+PAL.line+';border-radius:8px;padding:7px 9px"><div style="flex:0 0 18px;height:18px;border-radius:50%;background:'+col+';color:#0b0e14;font-size:10px;font-weight:800;text-align:center;line-height:18px">'+(i+1)+'</div><div style="font-size:10px">'+rc+'</div></div>';
    });
  } else {
    h+=_await('Awaiting the LLM review for pattern discoveries and ranked recommendations. Trigger it by saving today\u2019s data (\uD83D\uDCE5 on the dashboard) into AI Drive daily-data/, or paste the recorder JSON to the assistant.');
  }
  return h;
}
...
function timelineSvg(sym,st){
  ...
  if(snaps.length<2) return '<svg width="300" height="60"><text x="6" y="30" fill="'+PAL.sub+'" font-size="10">not enough bars yet</text></svg>';
  var W=Math.max(600, snaps.length*7), H=150, padT=22, padB=24;
  var ks=[],ps=[]; snaps.forEach(function(s){ if(typeof s.king==='number')ks.push(s.king); if(typeof s.px==='number')ps.push(s.px); });
  var all=ks.concat(ps); var lo=Math.min.apply(null,all), hi=Math.max.apply(null,all); if(hi===lo)hi=lo+1;
  function X(i){ return padT/2 + (i/(snaps.length-1))*(W-padT); }
  function Y(v){ return padT + (1-(v-lo)/(hi-lo))*(H-padT-padB); }
  var svg='<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'" style="display:block">';
  // S/R dominance band per bar
  snaps.forEach(function(s,i){ if(!s.sig||!s.sig.srb) return; var x0=X(i)-3;
    var c=(s.sig.srb.dom==='support')?'#0e1a13':(s.sig.srb.dom==='resistance')?'#1c1113':'transparent';
    svg+='<rect x="'+x0.toFixed(1)+'" y="0" width="7" height="'+H+'" fill="'+c+'"/>'; });
  // King staircase
  var kp=''; snaps.forEach(function(s,i){ if(typeof s.king!=='number')return; kp+=(kp?' L ':'M ')+X(i).toFixed(1)+' '+Y(s.king).toFixed(1); });
  svg+='<path d="'+kp+'" fill="none" stroke="'+PAL.gold+'" stroke-width="2"/>';
  // price line
  var pp=''; snaps.forEach(function(s,i){ if(typeof s.px!=='number')return; pp+=(pp?' L ':'M ')+X(i).toFixed(1)+' '+Y(s.px).toFixed(1); });
  svg+='<path d="'+pp+'" fill="none" stroke="#c8d3df" stroke-width="1.3" opacity="0.85"/>';
  // crossover markers
  snaps.forEach(function(s,i){ if(s.sig&&s.sig.srb&&s.sig.srb.cross){ var c=s.sig.srb.cross==='bears'?PAL.shortAccent:PAL.longAccent;
    svg+='<circle cx="'+X(i).toFixed(1)+'" cy="'+Y(s.px||s.king).toFixed(1)+'" r="3.5" fill="'+c+'"/>'; } });
  svg+='<text x="6" y="'+(H-6)+'" fill="'+PAL.gold+'" font-size="9">\u2501 King</text>'+
       '<text x="70" y="'+(H-6)+'" fill="#c8d3df" font-size="9">\u2501 price</text>'+
       '<text x="140" y="'+(H-6)+'" fill="'+PAL.longAccent+'" font-size="9">\u25ae support</text>'+
       '<text x="220" y="'+(H-6)+'" fill="'+PAL.shortAccent+'" font-size="9">\u25ae resistance</text>';
  svg+='</svg>'; return svg;
}
...
function convergenceSvg(sym,st){
  ...
  if(gaps.length<2) return '<svg width="300" height="50"><text x="6" y="26" fill="'+PAL.sub+'" font-size="10">not enough bars yet</text></svg>';
  var W=Math.max(600, gaps.length*7), H=80, mid=40;
  var mx=1; gaps.forEach(function(p){ mx=Math.max(mx,Math.abs(p.g)); });
  function X(i){ return 4+(i/(gaps.length-1))*(W-8); }
  function Y(g){ return mid - (g/mx)*(mid-8); }
  var svg='<svg width="'+W+'" height="'+H+'" viewBox="0 0 '+W+' '+H+'" style="display:block">';
  svg+='<line x1="0" y1="'+mid+'" x2="'+W+'" y2="'+mid+'" stroke="'+PAL.line+'" stroke-dasharray="3 3"/>';
  svg+='<text x="4" y="12" fill="'+PAL.sub+'" font-size="8">King above price (+)</text>';
  ...
  svg+='<path d="'+p+'" fill="none" stroke="#b58ce0" stroke-width="1.8"/>';
  svg+='</svg>'; return svg;
}
