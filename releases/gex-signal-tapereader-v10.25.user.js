...
console.log('[GPTS] v10.25 part1 loaded');
...
function nodeMapBlock(){
  ...
  function row(L){
    ...
    var arrow = L.state==='Building'?'\u2197':(L.state==='Fading'?'\u2198':'\u2192');
    var arrCol = L.side==='above' ? (L.state==='Building'?PAL.shortAccent:(L.state==='Fading'?PAL.longAccent:PAL.sub))
                                  : (L.state==='Building'?PAL.longAccent:(L.state==='Fading'?PAL.shortAccent:PAL.sub));
    ...
    return '<div style="display:flex;align-items:center;justify-content:space-between;font-size:10px;padding:2px 5px;border-radius:5px;background:'+bg+'">'+
      '<span style="width:58px;font-weight:800;font-variant-numeric:tabular-nums;color:'+(L.isKing?PAL.gold:PAL.ink)+'">'+marks+fmtNum(L.k)+'</span>'+
      '<span style="width:44px;text-align:right;color:'+PAL.sub+';font-variant-numeric:tabular-nums">'+L.pct+'%</span>'+
      '<span style="width:22px;text-align:center;color:'+arrCol+';font-weight:800">'+arrow+'</span>'+
      stageCell+
