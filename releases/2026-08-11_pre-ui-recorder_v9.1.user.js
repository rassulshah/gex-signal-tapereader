...
console.log('[GPTS] v9.1 part1 loaded');
...
function sectionHdrRight(text, rightHtml, tip){
  ...
  return '<div'+t+' style="background:'+PAL.card+';color:'+PAL.ink+';font-size:11px;font-weight:800;'+
    'letter-spacing:0.5px;padding:4px 9px;margin:5px 0 3px 0;border-left:3px solid '+PAL.blue+';'+
...
function symSignalsHdr(sym){
  ...
  return '<div title="'+tip.replace(/"/g,'')+'" style="background:'+PAL.card+';color:'+PAL.ink+';font-size:11px;font-weight:800;'+
    'letter-spacing:0.5px;padding:4px 9px;margin:5px 0 3px 0;border-left:3px solid '+PAL.blue+';'+
...
function accumBlock(){
  ...
  var px = STATE.SPY.price;
  function stColor(lbl){ return lbl==='Building'?PAL.longAccent:(lbl==='Fading'?PAL.shortAccent:PAL.blue); }
  ...
  if(inPlay){
    ...
    hdrRight = '<span title="Node most likely to matter next (in play)." style="color:'+ipColor+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+ipColor+';border-radius:20px">'+abbr+' '+fmtNum(inPlay.k)+'</span>'+
      (ipState?'<span title="Flow state of the in-play node (Skylit vocabulary)." style="color:'+stColor(ipRaw)+';font-weight:700;font-size:10px;padding:1px 7px;border:1px solid '+stColor(ipRaw)+';border-radius:20px">'+ipState+ipRapidGlyph+'</span>':'');
  }
  ...
  function strongLine(c, sideWord, forming){
    if(!c) return '';
    var col = c.side==='above' ? PAL.shortAccent : PAL.longAccent;
  ...
  function stateColor(label){
    return label==='Building' ? PAL.longAccent : (label==='Fading' ? PAL.shortAccent : PAL.blue);
  }
  ...
  function rowHtml(r){
    ...
    var col = stateColor(r.state.label);
