...
// ---- 8 · ONE REGISTER --------------------------------------------------------------------------
{
  ...
  let l=preregList();
  ok(l.length===10 && typeof l[0].pickFn==='function' && l[0].pickFn({gradeA:{n:3}}).n===3 && l.every(H=>typeof H.pickFn==='function'),'8d with nothing fetched the seed is used and picks resolve to functions (v15.55: 7 rows, H6/H7 added; v15.72: 9 rows, H8/H9; v15.80: 10 rows, H10 the rolling floor / ceiling)');
